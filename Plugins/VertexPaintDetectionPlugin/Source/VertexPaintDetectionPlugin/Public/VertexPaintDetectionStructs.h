...
enum class EVertexPaintWithinAreaFallOffType : uint8 {
	...
	SphericalFromCenter = 0 UMETA(DisplayName = "Spherical From Center", Tooltip = ""),
	GradiantUpward = 1 UMETA(DisplayName = "Gradiant Upward", Tooltip = "Falloff Up in World Space. "),
	GradiantDownward = 2 UMETA(DisplayName = "Gradiant Downward", Tooltip = "Falloff Down in World Space. "),
};
...
struct FVertexPaintDebugSettings {
	...
	float drawGetClosestVertexDataDebugSymbolsDuration = 2;
	...
	float drawGetEstimatedColorAtHitLocationDebugSymbolsDuration = 2;
	...
	float drawPaintWithinAreaDebugSymbolsDuration = 0.15f;
};
...
struct FVertexDetectFundementalStruct {
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Runtime Vertex Color Paint and Detection Plugin", meta = (ToolTip = "The Debug Settings for this Task, you can hover over each one to read more about it! \nNOTE Some of them requires you to turn of multithreading for the task job, since we can't paint debug symbols on an async thread.  "))
...
struct FVertexDetectPaintAtLocationAreaSettings {
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Runtime Vertex Color Paint and Detection Plugin", meta = (ToolTip = "Min Area of Effect of the Paint Job"))
	float areaOfEffectMin = 0;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Runtime Vertex Color Paint and Detection Plugin", meta = (ToolTip = "Max Area of Effect of the Paint Job"))
	float areaOfEffectMax = 100;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Runtime Vertex Color Paint and Detection Plugin", meta = (ToolTip = "Which type of FallOff we should use. The Default is the most common use case. Hover over each of them to get more info of what they do. "))
	EVertexPaintAtLocationFallOffType fallOffType = EVertexPaintAtLocationFallOffType::OutwardFallOff;
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Runtime Vertex Color Paint and Detection Plugin", meta = (ToolTip = "From which distance out from the centre we should calculate the falloff from. Depending on the FallOff type it has different use cases, but with the default one fall off type it's useful if you want to define an area that should have no falloff, for instance 50, then anything from 50 and below will not have falloff but it will go outwardly, instead doing that based on the centre. With Spherical fall off you get falloff toward the min and max AoE, so if you set this to be in between the Min and Max you can make more crater like shapes. "))
	float distanceToBaseFalloffFrom = 0;
...
struct FVertexPainthWithinAreaStruct : public FVertexPaintColorSettingStruct {
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Runtime Vertex Color Paint and Detection Plugin", meta = (ToolTip = "Similar to Paint At Locations distanceToBaseFalloffFrom, with this we can set how far away from center (or the bottom / top depending on the falloff type) that we should start the falloff. So if for instance falloff type is Spherical From Center, and this is set to 50, then within 50cm from the center we will paint with the actual paint strength set, and after that the fall off will start. \nMay be good if you for instance have a Torch and you want within a distance from the centre to always melt snow at the paint strength. "))
	float distanceToBaseFalloffFrom = 0;
