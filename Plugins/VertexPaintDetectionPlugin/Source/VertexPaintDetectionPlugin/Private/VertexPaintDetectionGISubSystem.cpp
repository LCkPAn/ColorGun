...
void UVertexPaintDetectionGISubSystem::Initialize(FSubsystemCollectionBase& Collection) {
	...
	if (cachedPhysicalMaterialAssets.Num() <= 0) {
		...
		UE_LOG(LogTemp, Warning, TEXT("Runtime Vertex Color Paint and Detection Plugin - NOTE The Plugins Plugin Game Instance Subsystem couldn't find any Physical Material Assets that had any physical surface set, so you will not be able to use GetPhysicalMaterialUsingPhysicsSurface function. \nIf you don't have any Physical Materials with physics surfaces set, then you can disregard this, otherwise you can in the Project Settings->Packaging, add Additional Directories to Cook, you can add the folder that has all the Physical Materials, then they will always be included in a Build and be Accessible. "));
	}
}
...
void UVertexPaintDetectionGISubSystem::AddCalculateColorsTaskToQueue(FCalculateColorsInfo calculateColorsInfo) {
	...
	if (calculateColorsInfo.fundementalStruct.multiThreadSettings.useMultithreadingForCalculations && componentCalculateColorsQueueArray.calculateColorsQueueArray.Num() > 0) {
		...
		if (componentCalculateColorsQueueArray.calculateColorsQueueArray[0].vertexPaintSourceMesh != sourceMeshTemp) {
			...
			UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Task already exists for Skeletal Mesh but the current source mesh has changed from what was created on the task before. Clears the components task queue so a new can be created.")), FColor::Orange);
	...
	if (componentCalculateColorsQueueArray.calculateColorsQueueArray.Num() > 0) {
		...
		UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Adds another Task for Mesh with ID: %s   -   %i   -   Current Amount of Tasks: %i"), *calculateColorsInfo.vertexPaintComponent->GetName(), calculateColorsInfo.taskID, componentCalculateColorsQueueArray.calculateColorsQueueArray.Num()), FColor::Cyan);
	}
	...
	// If starting a new que for the mesh it starts the async right away
		...
		UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Starts a new Task Queue for Actor: %s and Mesh: %s with ID: %i"), *calculateColorsInfo.vertexPaintDetectionHitActor->GetName(), *calculateColorsInfo.vertexPaintComponent->GetName(), calculateColorsInfo.taskID), FColor::Cyan);
...
void UVertexPaintDetectionGISubSystem::StartCalculateColorsTask(const FCalculateColorsInfo& calculateColorsInfo) {
	...
	if (FPlatformProcess::SupportsMultithreading() && calculateColorsInfo.fundementalStruct.multiThreadSettings.useMultithreadingForCalculations) {
		...
		if (!vertexColorPaint_QueueThreadPool) {
			...
			UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Creating Thread Pool  -  Num of Threads: %i"), numberOfThreads_Local), FColor::Cyan);
		}

		onGoingAsyncTasks.Add(calculateColorsInfo.taskID, calculateColorsInfo);


		UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Starting Task on Async Thread")), FColor::Cyan);
		...
		if (calculateColorsInfo.fundementalStruct.multiThreadSettings.useMultithreadingForCalculations) {
			...
			UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to run Calculations on Multithread but platform does not support it. Falling back to Game Thread instead!")), FColor::Orange);
		}
		else {

			UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Start Synchronous Calculate Colors Task for Actor: %s"), *calculateColorsInfo.vertexPaintDetectionHitActor->GetName()), FColor::Cyan);
		}

		onGoingAsyncTasks.Add(calculateColorsInfo.taskID, calculateColorsInfo);

		UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Starting Task on Game Thread")), FColor::Cyan);
...
void UVertexPaintDetectionGISubSystem::CalculateColorsTaskFinished(const FCalculateColorsInfo& calculateColorsInfo, const EVertexPaintDetectionTaskResult& paintTaskResult, const TArray<FAsyncTaskDebugMessages>& calculateColorsResultMessage) {

	
	if (IsInGameThread()) {

		// Got a crash very rarely when printing with PrintString in async task so moved it so we print them all here when in game thread. 
		for (auto message : calculateColorsResultMessage) {

			UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, message.debugMessage, message.messageColor);
		}

		UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - CalculateColorsTaskFinished() for Task: %i"), calculateColorsInfo.taskID), FColor::Cyan);
	}
	else {

		UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - CalculateColorsTaskFinished() for Task: %i but NOT in Game Thread as it should!"), calculateColorsInfo.taskID), FColor::Red);
	}
	...
	if (!validTask_Local) {
		...
		UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Finished Task is not Valid anymore. Possibly a paint job that Sets the colors was created After this task, which makes this task invalid since there's no use for it's result. ")), FColor::Red);
	...
	if (IsValid(calculateColorsInfo.vertexPaintComponent) && (calculateColorsInfo.vertexPaintSourceMesh != sourceMeshTemp || paintTaskResult == EVertexPaintDetectionTaskResult::FailWithInvalidMesh)) {
		...
		UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Task Finished but the Components Source Mesh has been changed - Removes it from Calculate Colors Tasks Queue!")), FColor::Red);
	}
	...
	// If the component isn't valid anymore i.e. we can't just remove it from the TMap with .Remove since the component is the key, we loop through it and just removes any that is invalid. 
		...
		UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Task Finished but the Primitive Component is not Valid anymore - Removes it from Calculate Colors Paint / Detection Tasks Queues!")), FColor::Red);
	...
	if ((paintTaskResult == EVertexPaintDetectionTaskResult::Success || paintTaskResult == EVertexPaintDetectionTaskResult::Fail) && IsValid(calculateColorsInfo.vertexPaintComponent)) {
		...
		// If there is no tasks left for the mesh
			...
			UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Task Finished for Actor: %s   -   No Tasks left in Que for Actor   -   Meshes with Tasks in Total: %i"), *calculateColorsInfo.vertexPaintDetectionHitActor->GetName(), amountOfTasksTemp), FColor::Cyan);
		}
	}
	...
	if (IsValid(calculateColorsInfo.vertexPaintComponent)) {
		...
		if (IsValid(calculateColorsInfo.worldTaskWasCreatedIn)) {
			...
			if (calculateColorsInfo.worldTaskWasCreatedIn->IsGameWorld() && !calculateColorsInfo.worldTaskWasCreatedIn->bIsTearingDown) {
				...
				if (paintTaskResult == EVertexPaintDetectionTaskResult::Success) {
					...
					if (calculateColorsInfo.vertexPaintSourceMesh == sourceMeshTemp) {
						...
						if (validTask_Local) {
							...
							UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Task Success for Actor: %s  -  Runs Wrap Up!"), *calculateColorsInfo.vertexPaintDetectionHitActor->GetName()), FColor::Green);
						...
						UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Task Fail the Source Mesh has changed on the primitive component from the one that it had when the task was created. ")), FColor::Red);
					...
					UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Task Failed. ")), FColor::Red);
					...
					UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Task Failed - Invalid Mesh. Perhaps Source Mesh got switching during the calculation. ")), FColor::Red);
				...
				UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Task Failed. World isn't game world or is tearing down. ")), FColor::Red);
			...
			UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Task Failed, World not Valid anymore")), FColor::Red);
	...
	// If primitive comp isn't valid anymore
		...
		UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Task Failed. Primitive Component isn't valid anymore. ")), FColor::Red);
	...
	if (taskResult == EVertexPaintDetectionTaskResult::Success) {
		...
		if (calculateColorsInfo.closestVertexFound_Paint >= 0 || calculateColorsInfo.closestVertexFound_Detection >= 0) {
			...
			if (calculateColorsInfo.vertexPaintStaticMeshComponent) {
				...
				if (calculateColorsInfo.vertexPaintStaticMeshComponent->GetStaticMesh()->bAllowCPUAccess) {
					...
					if (calculateColorsInfo.vertexPaintStaticMeshComponent->GetStaticMesh()->GetRenderData()->LODResources[0].VertexBuffers.StaticMeshVertexBuffer.IsInitialized() && calculateColorsInfo.vertexPaintStaticMeshComponent->GetStaticMesh()->GetRenderData()->LODResources[0].VertexBuffers.StaticMeshVertexBuffer.TexCoordVertexBuffer.IsInitialized()) {
						...
						for (uint32 i = 0; i < calculateColorsInfo.vertexPaintStaticMeshComponent->GetStaticMesh()->GetRenderData()->LODResources[0].VertexBuffers.StaticMeshVertexBuffer.GetNumTexCoords(); i++) {
							...
							if (closestVertexUV_Painting_Local.Num() > 0) {
								...
								UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Static Mesh Component Closest Vertex UV at Channel for Painting: %s  -  %i"), *closestVertexUV_Painting_Local[i].ToString(), i), FColor::Cyan);
							}

							if (closestVertexUV_Detection_Local.Num() > 0) {

								UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Static Mesh Component Closest Vertex UV at Channel for Detection: %s  -  %i"), *closestVertexUV_Detection_Local[i].ToString(), i), FColor::Cyan);
							}
						}
					}
				}
					...
					UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Can't get Closest Vertex UV since Mesh CPU Access is False")), FColor::Orange);
				}
			}
				...
				if (LODData.StaticVertexBuffers.StaticMeshVertexBuffer.IsInitialized() && LODData.StaticVertexBuffers.StaticMeshVertexBuffer.TexCoordVertexBuffer.IsInitialized()) {
					...
					for (uint32 i = 0; i < LODData.StaticVertexBuffers.StaticMeshVertexBuffer.GetNumTexCoords(); i++) {
						...
						if (closestVertexUV_Painting_Local.Num() > 0) {
							...
							UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Skeletal Mesh Component Closest Vertex UV at Channel for Painting: %s  -  %i"), *closestVertexUV_Painting_Local[i].ToString(), i), FColor::Cyan);
						}

						if (closestVertexUV_Detection_Local.Num() > 0) {

							UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Skeletal Mesh Component Closest Vertex UV at Channel for Detection: %s  -  %i"), *closestVertexUV_Detection_Local[i].ToString(), i), FColor::Cyan);
						}
					}
				}
			}
		...
		if (IsValid(calculateColorsInfo.paintOnMeshColorSettings.actor)) {
			...
			if (calculateColorsInfo.amountOfPaintedColorsOfEachChannelAbove0.successfullyGotVerticesOfEachChannelAtTheMinAmount) {
				...
				if (calculateColorsInfo.amountOfPaintedColorsOfEachChannelAbove0.averageAmountOfRedColorAtTheMinAmount == 1 && calculateColorsInfo.amountOfPaintedColorsOfEachChannelAbove0.averageAmountOfGreenColorAtTheMinAmount == 1 && calculateColorsInfo.amountOfPaintedColorsOfEachChannelAbove0.averageAmountOfBlueColorAtTheMinAmount == 1 && calculateColorsInfo.amountOfPaintedColorsOfEachChannelAbove0.averageAmountOfAlphaColorAtTheMinAmount == 1) {
					...
					if (calculateColorsInfo.paintOnMeshColorSettings.applyVertexColorSettings.applyVertexColorSetting == EApplyVertexColorSetting::EAddVertexColor) {
						...
						if (calculateColorsInfo.paintOnMeshColorSettings.applyVertexColorSettings.redColor < 0 || calculateColorsInfo.paintOnMeshColorSettings.applyVertexColorSettings.greenColor < 0 || calculateColorsInfo.paintOnMeshColorSettings.applyVertexColorSettings.blueColor < 0 || calculateColorsInfo.paintOnMeshColorSettings.applyVertexColorSettings.alphaColor < 0) {
							...
							UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Apply Color with Add on an Already Fully Colored Mesh. So no visual difference will occur. ")), FColor::Orange);
						}
					}
					...
					// If Fully painted and set to SetVertexColors, then we just require that any of them are less than 1, since then there will be a difference
						...
						if (calculateColorsInfo.paintOnMeshColorSettings.applyVertexColorSettings.redColor < 1 || calculateColorsInfo.paintOnMeshColorSettings.applyVertexColorSettings.greenColor < 1 || calculateColorsInfo.paintOnMeshColorSettings.applyVertexColorSettings.blueColor < 1 || calculateColorsInfo.paintOnMeshColorSettings.applyVertexColorSettings.alphaColor < 1) {
							...
							UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Apply Color with Set on an Already Fully Colored Mesh. So no visual difference will occur. ")), FColor::Orange);
						}
					}
				}
					...
					if (calculateColorsInfo.paintOnMeshColorSettings.applyVertexColorSettings.applyVertexColorSetting == EApplyVertexColorSetting::EAddVertexColor) {
						...
						if (calculateColorsInfo.paintOnMeshColorSettings.applyVertexColorSettings.redColor > 0 || calculateColorsInfo.paintOnMeshColorSettings.applyVertexColorSettings.greenColor > 0 || calculateColorsInfo.paintOnMeshColorSettings.applyVertexColorSettings.blueColor > 0 || calculateColorsInfo.paintOnMeshColorSettings.applyVertexColorSettings.alphaColor > 0) {
							...
							UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Remove Colors with Add on a Mesh that already has all of them removed. So no visual difference will occur. ")), FColor::Orange);
						}
					}
						...
						if (calculateColorsInfo.paintOnMeshColorSettings.applyVertexColorSettings.redColor > 0 || calculateColorsInfo.paintOnMeshColorSettings.applyVertexColorSettings.greenColor > 0 || calculateColorsInfo.paintOnMeshColorSettings.applyVertexColorSettings.blueColor > 0 || calculateColorsInfo.paintOnMeshColorSettings.applyVertexColorSettings.alphaColor > 0) {
							...
							UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Remove Colors with Set on a Mesh that already has all of them removed. So no visual difference will occur. ")), FColor::Orange);
						}
					}
				}
				...
				if (calculateColorsInfo.fundementalStruct.debugSettings.printLogsToScreen || calculateColorsInfo.fundementalStruct.debugSettings.printLogsToOutputLog) {
					...
					if (calculateColorsInfo.amountOfPaintedColorsOfEachChannelAbove0.averageAmountOfRedColorAtTheMinAmount == 1) {
						...
						if (calculateColorsInfo.paintOnMeshColorSettings.applyVertexColorSettings.redColor > 0) {
							...
							UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Apply Red Vertex Color but Red is already Full Color")), FColor::Orange);
						}
					}
						...
						if (calculateColorsInfo.paintOnMeshColorSettings.applyVertexColorSettings.redColor < 0) {
							...
							UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Remove Red Vertex Color but Red is Completely Removed")), FColor::Orange);
						}
					}
					...
					if (calculateColorsInfo.amountOfPaintedColorsOfEachChannelAbove0.averageAmountOfGreenColorAtTheMinAmount == 1) {
						...
						if (calculateColorsInfo.paintOnMeshColorSettings.applyVertexColorSettings.greenColor > 0) {
							...
							UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Apply Green Vertex Color but Green is already Full Color! ")), FColor::Orange);
						}
					}
						...
						if (calculateColorsInfo.paintOnMeshColorSettings.applyVertexColorSettings.greenColor < 0) {
							...
							UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Remove Green Vertex Color but Green is Completely Removed")), FColor::Orange);
						}
					}
					...
					if (calculateColorsInfo.amountOfPaintedColorsOfEachChannelAbove0.averageAmountOfBlueColorAtTheMinAmount == 1) {
						...
						if (calculateColorsInfo.paintOnMeshColorSettings.applyVertexColorSettings.blueColor > 0) {
							...
							UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Apply Blue Vertex Color but Blue is already Full Color! ")), FColor::Orange);
						}
					}
						...
						if (calculateColorsInfo.paintOnMeshColorSettings.applyVertexColorSettings.blueColor < 0) {
							...
							UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Remove Blue Vertex Color but Blue is Completely Removed")), FColor::Orange);
						}
					}
					...
					if (calculateColorsInfo.amountOfPaintedColorsOfEachChannelAbove0.averageAmountOfAlphaColorAtTheMinAmount == 1) {
						...
						if (calculateColorsInfo.paintOnMeshColorSettings.applyVertexColorSettings.alphaColor > 0) {
							...
							UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Apply Alpha Vertex Color but Alpha is already Full Color! ")), FColor::Orange);
						}
					}
						...
						if (calculateColorsInfo.paintOnMeshColorSettings.applyVertexColorSettings.alphaColor < 0) {
							...
							UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Remove Alpha Vertex Color but Alpha is Completely Removed")), FColor::Orange);
						}
					}
...
void UVertexPaintDetectionGISubSystem::GetClosestVertexDataOnMesh_WrapUp(const FCalculateColorsInfo& calculateColorsInfo, const FVertexDetectInfoStruct& detectStruct, const FColor& closestColor, const FColor& avarageColor, const FLinearColor& estimatedColorAtHitLocation, const TArray<FVector2D>& closestVertexUV_Detection) {
	...
	UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Get Closest Vertex Data on Mesh Wrap Up")), FColor::Green);
	...
	if (calculateColorsInfo.closestVertexFound_Detection == -1) {
		...
		UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Was unable to get Closest Vertex Data for Detection")), FColor::Red);
	}

	else {

		UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Closest Vertex Index Found for Detection: %i"), calculateColorsInfo.closestVertexFound_Detection), FColor::Cyan);

		UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Average Color in Detection: %s"), *avarageColorInArea_Local.avarageVertexColorsWithinAreaOfEffect.ToString()), FColor::Cyan);
	...
	if (closestVertexDataResult_Local.closestVertexDataSuccessfullyAcquired) {
		...
		UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Successfully Got Closest Mesh Data!")), FColor::Green);
	}

	else {

		UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Got Mesh Data but NOT the Closest Vertex Data From Hit Mesh which we wanted to get. Check if everything is correctly hooked up in the Detection Node!")), FColor::Orange);
	}
...
void UVertexPaintDetectionGISubSystem::GetAllVertexColorsOnly_WrapUp(const FCalculateColorsInfo& calculateColorsInfo) {
	...
	UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Get All Vertex Colors Only Wrap Up")), FColor::Green);
...
void UVertexPaintDetectionGISubSystem::PaintOnMeshAtLocation_WrapUp(const FCalculateColorsInfo& calculateColorsInfo, const TArray<FVector2D>& closestVertexUV_Painting) {
	...
	UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint on Mesh at Location Wrap Up")), FColor::Green);
	...
	if (calculateColorsInfo.closestVertexFound_Paint == -1) {
		...
		UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint at Location - Was unable to get Closest Vertex Index")), FColor::Red);
	}

	else {

		UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Get Closest Vertex Data - Closest Vertex Index Found for Painting: %i"), calculateColorsInfo.closestVertexFound_Paint), FColor::Cyan);
	}
	...
	UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Average Color in Paint at Location Area: %s"), *avarageColorInArea_Local.avarageVertexColorsWithinAreaOfEffect.ToString()), FColor::Green);


	PrintClosestVertexColorResults(closestVertexDataResult_Local, estimatedColorAtHitLocationResult_Local, calculateColorsInfo.paintOnMeshAtLocationSettings.paintAtAreaSettings.areaOfEffectMax, calculateColorsInfo);
	...
	if (calculateColorsInfo.shouldApplyVertexColors) {
		...
		if (calculateColorsInfo.vertexPaintSettings.applyPaintJobToVertexColors) {
			...
			UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint on Mesh at Location Success - A change to colors was made. ")), FColor::Green);
		}

		else {

			UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint on Mesh at Location Success - A change to colors was made, but it's set to Not apply the paint job to vertex colors!")), FColor::Green);
		}
	}
		...
		UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint on Mesh at Location Success - But there was no change in colors. Perhaps the mesh is already fully colored if you tried to add, or it is already fully removed if you tried to remove.")), FColor::Green);
	}
...
void UVertexPaintDetectionGISubSystem::PaintWithinArea_WrapUp(const FCalculateColorsInfo& calculateColorsInfo) {
	...
	UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint Within Area Wrap Up")), FColor::Green);
	...
	if (calculateColorsInfo.shouldApplyVertexColors) {
		...
		if (calculateColorsInfo.vertexPaintSettings.applyPaintJobToVertexColors) {
			...
			UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint on Mesh Within Area Success - A change to colors was made.")), FColor::Green);
		}

		else {

			UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint on Mesh Within Area Success - A change to colors was made, but it's set to Not apply the paint job to vertex colors!")), FColor::Green);
		}
	}

	else {

		UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint on Mesh Within Area Success - But no change in colors was made. Perhaps the mesh is already fully colored if you tried to add, or it is already fully removed if you tried to remove.")), FColor::Green);
	}


	if (!calculateColorsInfo.paintWithinArea_TriedToApplyColorOnAnyVertex) {

		UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint on Mesh Within Area Success - But no Vertex on the mesh we tried to paint on was inside any of the componentsToCheckIfWithin. If using complex shape, make sure the componentToCheckIfWithin that is set to use the complex shape has some collision. ")), FColor::Orange);
	}
...
void UVertexPaintDetectionGISubSystem::PaintOnEntireMesh_WrapUp(const FCalculateColorsInfo& calculateColorsInfo) {
	...
	UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint Entire Mesh Wrap Up")), FColor::Green);
	...
	if (calculateColorsInfo.shouldApplyVertexColors) {
		...
		if (calculateColorsInfo.vertexPaintSettings.applyPaintJobToVertexColors) {
			...
			UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint on Entire Mesh Success - A change to colors was made.")), FColor::Green);
		}

		else {

			UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint on Entire Mesh Success - A change to colors was made, but it's set to Not apply the paint job to vertex colors!")), FColor::Green);
		}
	}
...
void UVertexPaintDetectionGISubSystem::PaintColorSnippet_WrapUp(const FCalculateColorsInfo& calculateColorsInfo) {
	...
	UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint Color Snippet Wrap Up")), FColor::Green);
	...
	if (calculateColorsInfo.shouldApplyVertexColors) {
		...
		if (calculateColorsInfo.vertexPaintSettings.applyPaintJobToVertexColors) {
			...
			UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint Color Snippet Success")), FColor::Green);
		}
		else {

			UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint Color Snippet Success, but it's set to Not apply the paint job to vertex colors!")), FColor::Green);
		}
	}

	else {

		UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint Color Snippet Success but no Colors was Changed, is the same snippet being painted multiple times?")), FColor::Green);
	}
...
void UVertexPaintDetectionGISubSystem::ApplyColorsDirectly_WrapUp(const FCalculateColorsInfo& calculateColorsInfo) {
	...
	UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Apply Colors Directly Wrap Up")), FColor::Green);
	...
	if (calculateColorsInfo.shouldApplyVertexColors) {
		...
		if (calculateColorsInfo.vertexPaintSettings.applyPaintJobToVertexColors) {
			...
			UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Apply Colors Directly Success - A change to colors was made")), FColor::Green);
		}
		else {

			UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Apply Colors Directly Success - A change to colors was made, but it's set to Not apply the paint job to vertex colors!")), FColor::Green);
		}
	}
...
FVertexDetectPhysicsSurfaceDataStruct UVertexPaintDetectionGISubSystem::GetPhysicsSurfaceData(const FColor& vertexColor, const FCalculateColorsInfo& calculateColorsInfo, UMaterialInterface* material) {
	...
	if (!IsValid(material)) {
		...
		UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Couldn't Get Physics Surface Data because couldn't get material at closest vertex to hit location. ")), FColor::Orange);
	...
	if (!UVertexPaintFunctionLibrary::GetVertexPaintMaterialDataAsset()) {
		...
		UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Can't get Physics Surface Data because a Vertex Paint Material Data Asset hasn't been set in the Project Settings. ")), FColor::Orange);
...
void UVertexPaintDetectionGISubSystem::PrintClosestVertexColorResults(const FVertexDetectClosestVertexDataResultStruct& closestVertexDataResult, const FVertexDetectEstimatedColorAtHitLocationResultStruct& estimatedColorAtHitLocationResult, const float& avarageColorAreaOfEffect, const FCalculateColorsInfo& calculateColorsInfo) {
	...
	// Closest Color
	UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Closest Vertex Color: %s"), *closestVertexDataResult.closestVertexGeneralInfo.closestVertexColors.ToString()), FColor::Green);

	// Estimated Color at Hit Location
	if (estimatedColorAtHitLocationResult.estimatedColorAtHitLocationDataSuccessfullyAcquired)
		UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Estimated Hit Location Color: %s"), *estimatedColorAtHitLocationResult.estimatedColorAtHitLocation.ToString()), FColor::Green);
	...
	// Closest Normal
	UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Closest Vertex Normal World: %s  -  Closest Vertex Normal Local: %s"), *closestVertexDataResult.closestVertexGeneralInfo.closestVertexNormalInfo.closestVertexNormal.ToString(), *closestVertexDataResult.closestVertexGeneralInfo.closestVertexNormalInfo.closestVertexNormalLocal.ToString()), FColor::Cyan);

	// Closest Section
	if (closestVertexDataResult.closestVertexGeneralInfo.closestSection != -1)
		UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Closest Vertex Section: %i"), closestVertexDataResult.closestVertexGeneralInfo.closestSection), FColor::Cyan);

	// Closest Material
	if (closestVertexDataResult.closestVertexGeneralInfo.closestVertexMaterial)
		UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Closest Material: %s"), *closestVertexDataResult.closestVertexGeneralInfo.closestVertexMaterial->GetName()), FColor::Cyan);

	// Closest Surfaces
	if (closestVertexDataResult.closestVertexPhysicalSurfaceInfo.physicsSurfaceSuccessfullyAcquired) {

		UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Physics Surface and Color Values at Closest Vertex: Default: %s: %f  -  Red: %s: %f  -  Green: %s: %f  -  Blue: %s: %f  -  Alpha: %s: %f"), *closestVertexDataResult.closestVertexPhysicalSurfaceInfo.physicsSurface_AtDefault.physicalSurfaceAsStringAtDefault, closestVertexDataResult.closestVertexPhysicalSurfaceInfo.physicsSurface_AtDefault.physicalSurfaceValueAtDefault, *closestVertexDataResult.closestVertexPhysicalSurfaceInfo.physicsSurface_AtRed.physicalSurfaceAsStringAtRedChannel, closestVertexDataResult.closestVertexPhysicalSurfaceInfo.physicsSurface_AtRed.physicalSurfaceValueAtRedChannel, *closestVertexDataResult.closestVertexPhysicalSurfaceInfo.physicsSurface_AtGreen.physicalSurfaceAsStringAtGreenChannel, closestVertexDataResult.closestVertexPhysicalSurfaceInfo.physicsSurface_AtGreen.physicalSurfaceValueAtGreenChannel, *closestVertexDataResult.closestVertexPhysicalSurfaceInfo.physicsSurface_AtBlue.physicalSurfaceAsStringAtBlueChannel, closestVertexDataResult.closestVertexPhysicalSurfaceInfo.physicsSurface_AtBlue.physicalSurfaceValueAtBlueChannel, *closestVertexDataResult.closestVertexPhysicalSurfaceInfo.physicsSurface_AtAlpha.physicalSurfaceAsStringAtAlphaChannel, closestVertexDataResult.closestVertexPhysicalSurfaceInfo.physicsSurface_AtAlpha.physicalSurfaceValueAtAlphaChannel), FColor::Green);
	}

	// If couldn't get physical surface but just closest colors
	else {

		UVertexPaintFunctionLibrary::PrintTaskLog(calculateColorsInfo.fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Couldn't get Physical Surfaces at Closest Vertex, perhaps the Hit Material isn't registered in the Material Data Asset?")), FColor::Orange);
	}

	// Print Debug Box and Directional Arrow
	if (calculateColorsInfo.fundementalStruct.debugSettings.drawGetClosestVertexDataDebugSymbols) {

		DrawDebugBox(GetWorld(), closestVertexDataResult.closestVertexGeneralInfo.closestVertexPositionInfo.closestVertexPositionWorld, FVector(5, 5, 5), FColor::Red, false, calculateColorsInfo.fundementalStruct.debugSettings.drawGetClosestVertexDataDebugSymbolsDuration, 0, 3);

		DrawDebugDirectionalArrow(GetWorld(), closestVertexDataResult.closestVertexGeneralInfo.closestVertexPositionInfo.closestVertexPositionWorld, (closestVertexDataResult.closestVertexGeneralInfo.closestVertexPositionInfo.closestVertexPositionWorld + (closestVertexDataResult.closestVertexGeneralInfo.closestVertexNormalInfo.closestVertexNormal * 100)), 50, FColor::Red, false, calculateColorsInfo.fundementalStruct.debugSettings.drawGetClosestVertexDataDebugSymbolsDuration, 0, 3);

		if (avarageColorAreaOfEffect > 0) {

			DrawDebugSphere(GetWorld(), closestVertexDataResult.closestVertexGeneralInfo.closestVertexPositionInfo.closestVertexPositionWorld, avarageColorAreaOfEffect, 16, FColor::Green, false, calculateColorsInfo.fundementalStruct.debugSettings.drawGetClosestVertexDataDebugSymbolsDuration, 0, 3);
		}
	}
	...
	if (estimatedColorAtHitLocationResult.estimatedColorAtHitLocationDataSuccessfullyAcquired) {
		...
		if (calculateColorsInfo.fundementalStruct.debugSettings.drawGetEstimatedColorAtHitLocationDebugSymbols) {


			DrawDebugBox(calculateColorsInfo.vertexPaintDetectionHitActor->GetWorld(), estimatedColorAtHitLocationResult.worldSpaceLocationWeEstimatedTheColorAt, FVector(5, 5, 5), FQuat::Identity, FColor::Emerald, false, calculateColorsInfo.fundementalStruct.debugSettings.drawGetEstimatedColorAtHitLocationDebugSymbolsDuration, 0, 3);


			DrawDebugBox(calculateColorsInfo.vertexPaintDetectionHitActor->GetWorld(), closestVertexDataResult.closestVertexGeneralInfo.closestVertexPositionInfo.closestVertexPositionWorld, FVector(5, 5, 5), FQuat::Identity, FColor::Silver, false, calculateColorsInfo.fundementalStruct.debugSettings.drawGetEstimatedColorAtHitLocationDebugSymbolsDuration, 0, 3);


			if (calculateColorsInfo.estimatedColorAtHitLocation_VertexToLerpToward_Paint >= 0) {

				// Hit Location
				DrawDebugBox(calculateColorsInfo.vertexPaintDetectionHitActor->GetWorld(), calculateColorsInfo.paintOnMeshAtLocationSettings.hitFundementals.hitLocation, FVector(5, 5, 5), FQuat::Identity, FColor::Purple, false, calculateColorsInfo.fundementalStruct.debugSettings.drawGetEstimatedColorAtHitLocationDebugSymbolsDuration, 0, 3);

				// Vertex we Lerp Toward
				DrawDebugBox(calculateColorsInfo.vertexPaintDetectionHitActor->GetWorld(), calculateColorsInfo.estimatedColorAtHitLocation_VertexToLerpToward_LocationInWorldSpace_Paint, FVector(5, 5, 5), FQuat::Identity, FColor::Yellow, false, calculateColorsInfo.fundementalStruct.debugSettings.drawGetEstimatedColorAtHitLocationDebugSymbolsDuration, 0, 3);

				// calculateColorsInfo.estimated
				DrawDebugDirectionalArrow(calculateColorsInfo.vertexPaintDetectionHitActor->GetWorld(), calculateColorsInfo.closestVertexPositionToHitLocationAtMinLOD_World_Paint, calculateColorsInfo.paintOnMeshAtLocationSettings.hitFundementals.hitLocation, 10, FColor::Purple, false, calculateColorsInfo.fundementalStruct.debugSettings.drawGetEstimatedColorAtHitLocationDebugSymbolsDuration, 0, 5);
				...
				for (auto vertexWorldLocationTemp : calculateColorsInfo.estimatedColorAtHitLocation_ClosestVertexWorldLocationsToTheHitLocation_Paint) {
					...
					DrawDebugBox(calculateColorsInfo.vertexPaintDetectionHitActor->GetWorld(), vertexWorldLocationTemp, FVector(2.5f, 2.5f, 2.5f), FQuat::Identity, FColor::Blue, false, calculateColorsInfo.fundementalStruct.debugSettings.drawGetEstimatedColorAtHitLocationDebugSymbolsDuration, 0, 3);
				}
			}
			...
			if (calculateColorsInfo.estimatedColorAtHitLocation_VertexToLerpToward_Detect >= 0) {
				...
				// Hit Location
				DrawDebugBox(calculateColorsInfo.vertexPaintDetectionHitActor->GetWorld(), calculateColorsInfo.detectInfoSettings.hitFundementals.hitLocation, FVector(5, 5, 5), FQuat::Identity, FColor::Purple, false, calculateColorsInfo.fundementalStruct.debugSettings.drawGetEstimatedColorAtHitLocationDebugSymbolsDuration, 0, 3);

				// Vertex we Lerp Toward
				DrawDebugBox(calculateColorsInfo.vertexPaintDetectionHitActor->GetWorld(), calculateColorsInfo.estimatedColorAtHitLocation_VertexToLerpToward_LocationInWorldSpace_Detect, FVector(5, 5, 5), FQuat::Identity, FColor::Yellow, false, calculateColorsInfo.fundementalStruct.debugSettings.drawGetEstimatedColorAtHitLocationDebugSymbolsDuration, 0, 3);

				// Direction from closest vertex to hit location
				DrawDebugDirectionalArrow(calculateColorsInfo.vertexPaintDetectionHitActor->GetWorld(), calculateColorsInfo.closestVertexPositionToHitLocationAtMinLOD_World_Detect, calculateColorsInfo.detectInfoSettings.hitFundementals.hitLocation, 10, FColor::Purple, false, calculateColorsInfo.fundementalStruct.debugSettings.drawGetEstimatedColorAtHitLocationDebugSymbolsDuration, 0, 5);
...
void UVertexPaintDetectionGISubSystem::AddColorVertexBufferToBufferCleanup(UWorld* world, FColorVertexBuffer* colorVertexBuffer) {
	...
	world->GetTimerManager().SetTimer(cleanupOldVertexColorBufferTimers[cleanupOldVertexColorBufferTimers.Num() - 1], this, &UVertexPaintDetectionGISubSystem::CleanUpOldVertexColorBuffer, 0.5, false);
}
