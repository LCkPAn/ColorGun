...
void UVertexPaintDetectionComponent::GetClosestVertexDataOnMesh(FVertexDetectInfoStruct getClosestVertexDataStruct, FVertexDetectAdditionalDataToPassThrough additionalDataToPassThrough) {
	...
	if (!UVertexPaintFunctionLibrary::CheckValidFundementals(getClosestVertexDataStruct, getClosestVertexDataStruct.meshComponent))
		passedChecks_Local = false;

	if (passedChecks_Local && UVertexPaintFunctionLibrary::CheckHaveReachedMaxDetectionTask(getClosestVertexDataStruct))
		passedChecks_Local = false;
	...
	if (passedChecks_Local) {
		...
		UVertexPaintFunctionLibrary::PrintTaskLog(getClosestVertexDataStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Check Valid Failed at Get Closest Vertex Data On Mesh!")), FColor::Red);
	...
	UVertexPaintFunctionLibrary::PrintTaskLog(getClosestVertexDataStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Get Closest Vertex Data On Mesh CheckValid Successfull. ")), FColor::Green);

	// Sets this here if not already set instead of Begin Play, in case users make childs of this comp and overrides Begin Play and forget to call super
	if (!vertexPaintGameInstanceSubsystem_Global && UGameplayStatics::GetGameInstance(GetWorld()))
...
void UVertexPaintDetectionComponent::GetAllVertexColorsOnly(FVertexDetectGetColorsOnlyStruct getAllVertexColorsStruct, FVertexDetectAdditionalDataToPassThrough additionalDataToPassThrough) {
	...
	if (IsValid(getAllVertexColorsStruct.meshComponent)) {
		...
		if (IsValid(getAllVertexColorsStruct.meshComponent->GetOwner())) {
			...
			UVertexPaintFunctionLibrary::PrintTaskLog(getAllVertexColorsStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Get Colors Only on Actor: %s and Component: %s"), *getAllVertexColorsStruct.meshComponent->GetOwner()->GetName(), *getAllVertexColorsStruct.meshComponent->GetName()), FColor::Cyan);
		}
	}
	...
	if (!UVertexPaintFunctionLibrary::CheckValidFundementals(getAllVertexColorsStruct, getAllVertexColorsStruct.meshComponent))
		passedChecks_Local = false;

	if (passedChecks_Local && UVertexPaintFunctionLibrary::CheckHaveReachedMaxDetectionTask(getAllVertexColorsStruct))
		passedChecks_Local = false;


	if (!passedChecks_Local) {

		UVertexPaintFunctionLibrary::PrintTaskLog(getAllVertexColorsStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Check Valid Failed at Get All Vertex Colors Only!")), FColor::Red);

		UVertexPaintFunctionLibrary::RunGetAllColorsOnlyCallbacks(calculateColorsInfoTemp);
		return;
	}

	UVertexPaintFunctionLibrary::PrintTaskLog(getAllVertexColorsStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Get All Vertex Colors Only CheckValid Successfull. ")), FColor::Green);
...
void UVertexPaintDetectionComponent::PaintOnMeshAtLocation(FVertexPaintAtLocationStruct paintAtLocationStruct, FVertexDetectAdditionalDataToPassThrough additionalDataToPassThrough) {
	...
	if (IsValid(paintAtLocationStruct.meshComponent)) {
		...
		if (IsValid(paintAtLocationStruct.meshComponent->GetOwner())) {
			...
			UVertexPaintFunctionLibrary::PrintTaskLog(paintAtLocationStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Paint on Mesh at Location on Actor: %s and Component: %s"), *paintAtLocationStruct.meshComponent->GetOwner()->GetName(), *paintAtLocationStruct.meshComponent->GetName()), FColor::Cyan);
		}
	...
	paintAtLocationStruct.applyVertexColorSettings = UVertexPaintFunctionLibrary::ClampPaintStrengths(paintAtLocationStruct.applyVertexColorSettings);
	...
	paintAtLocationStruct.applyVertexColorSettings = UVertexPaintFunctionLibrary::ClampPaintLimits(paintAtLocationStruct.applyVertexColorSettings);
	...
	if (!UVertexPaintFunctionLibrary::CheckValidFundementals(paintAtLocationStruct, paintAtLocationStruct.meshComponent))
		passedChecks_Local = false;

	if (passedChecks_Local && !UVertexPaintFunctionLibrary::CheckValidVertexPaintSettings(paintAtLocationStruct))
		passedChecks_Local = false;

	if (passedChecks_Local && !UVertexPaintFunctionLibrary::CheckValidVertexPaintColorSettings(paintAtLocationStruct))
		passedChecks_Local = false;
	...
	if (passedChecks_Local) {
		...
		if (paintAtLocationStruct.paintAtAreaSettings.areaOfEffectMin > paintAtLocationStruct.paintAtAreaSettings.areaOfEffectMax) {


			UVertexPaintFunctionLibrary::PrintTaskLog(paintAtLocationStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Check Valid, Paint at Location Fail: Paint Area of Effect is > than areaOfEffectMax")), FColor::Red);

			passedChecks_Local = false;
		}


		if (paintAtLocationStruct.paintAtAreaSettings.areaOfEffectMin <= 0 && paintAtLocationStruct.paintAtAreaSettings.areaOfEffectMax <= 0) {

			UVertexPaintFunctionLibrary::PrintTaskLog(paintAtLocationStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Check Valid, Paint at Location Fail: Paint Area of Effect is <= 0")), FColor::Red);
	...
	if (!passedChecks_Local) {
		...
		UVertexPaintFunctionLibrary::PrintTaskLog(paintAtLocationStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Check Valid Failed at Paint on Mesh at Location!")), FColor::Red);
	...
	if (paintAtLocationStruct.applyVertexColorSettings.applyColorsUsingPhysicsSurface.applyVertexColorUsingPhysicsSurface) {
		...
		UVertexPaintFunctionLibrary::PrintTaskLog(paintAtLocationStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint on Mesh at Location CheckValid Successfull. Trying to Apply Color with EPhysicalSurface: %s  with Paint Strength: %f  -  Fall Back to RGBA Colors if Failed to Apply Colors Using Physics Surface: %i  -  Paint Strength on Surfaces Without the Physical Surface: %f"), *StaticEnum<EPhysicalSurface>()->GetDisplayNameTextByValue(paintAtLocationStruct.applyVertexColorSettings.applyColorsUsingPhysicsSurface.physicsSurfaceToApply.GetValue()).ToString(), paintAtLocationStruct.applyVertexColorSettings.applyColorsUsingPhysicsSurface.paintStrengthToApply, paintAtLocationStruct.applyVertexColorSettings.applyColorsUsingPhysicsSurface.fallbackToRGBAColorsIfFailedToApplyColorsUsingPhysicsSurface, paintAtLocationStruct.applyVertexColorSettings.applyColorsUsingPhysicsSurface.paintStrengtOnChannelsWithoutThePhysicsSurface), FColor::Green);
	}

	else {

		UVertexPaintFunctionLibrary::PrintTaskLog(paintAtLocationStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint on Mesh at Location CheckValid Successfull. Trying to Apply Color: %s  Area of Effect: %f  Falloff Strength: %f"), *FLinearColor(paintAtLocationStruct.applyVertexColorSettings.redColor, paintAtLocationStruct.applyVertexColorSettings.greenColor, paintAtLocationStruct.applyVertexColorSettings.blueColor, paintAtLocationStruct.applyVertexColorSettings.alphaColor).ToString(), paintAtLocationStruct.paintAtAreaSettings.areaOfEffectMin, paintAtLocationStruct.paintAtAreaSettings.fallOffStrength), FColor::Green);
	}
...
void UVertexPaintDetectionComponent::PaintOnMeshWithinArea(FVertexPainthWithinAreaStruct paintWithinAreaStruct, FVertexDetectAdditionalDataToPassThrough additionalDataToPassThrough) {
	...
	if (IsValid(paintWithinAreaStruct.meshComponent)) {
		...
		if (IsValid(paintWithinAreaStruct.meshComponent->GetOwner())) {
			...
			UVertexPaintFunctionLibrary::PrintTaskLog(paintWithinAreaStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Paint On Mesh Within Area on Actor: %s and Component: %s"), *paintWithinAreaStruct.meshComponent->GetOwner()->GetName(), *paintWithinAreaStruct.meshComponent->GetName()), FColor::Cyan);
		}
	}


	// Clamps it so if the user sets like 0.000001, we will paint at the lowest amount of 1 when converted to FColor which range from 0-255
	paintWithinAreaStruct.applyVertexColorSettings = UVertexPaintFunctionLibrary::ClampPaintStrengths(paintWithinAreaStruct.applyVertexColorSettings);
	...
	paintWithinAreaStruct.applyVertexColorSettings = UVertexPaintFunctionLibrary::ClampPaintLimits(paintWithinAreaStruct.applyVertexColorSettings);
	...
	if (!UVertexPaintFunctionLibrary::CheckValidFundementals(paintWithinAreaStruct, paintWithinAreaStruct.meshComponent))
		passedChecks_Local = false;

	if (passedChecks_Local && !UVertexPaintFunctionLibrary::CheckValidVertexPaintSettings(paintWithinAreaStruct))
		passedChecks_Local = false;

	if (passedChecks_Local && !UVertexPaintFunctionLibrary::CheckValidVertexPaintColorSettings(paintWithinAreaStruct))
		passedChecks_Local = false;
	...
	if (passedChecks_Local && paintWithinAreaStruct.componentsToCheckIfIsWithin.Num() <= 0) {
		...
		UVertexPaintFunctionLibrary::PrintTaskLog(paintWithinAreaStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint Within Area Check Valid Fail! It has no componentsToCheckIfIsWithin!")), FColor::Red);
			...
			if (IsValid(compToCheckWithinTemp.componentToCheckIfIsWithin)) {
				...
				if (Cast<ALandscape>(compToCheckWithinTemp.componentToCheckIfIsWithin->GetOwner())) {
					...
					if (compToCheckWithinTemp.paintWithinAreaShape != Enum_PaintWithinAreaShape::isComplexShape) {
						...
						UVertexPaintFunctionLibrary::PrintTaskLog(paintWithinAreaStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint Within Area Check Valid Failed since one of the componentToCheckIfWithin is a Landscape, but the PaintWithinArea Shape is not set to be Complex Shape which is required to check if you're within Landscapes!")), FColor::Red);
					...
					if (compToCheckWithinTemp.paintWithinAreaShape != Enum_PaintWithinAreaShape::isSphereShape) {
						...
						UVertexPaintFunctionLibrary::PrintTaskLog(paintWithinAreaStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint Within Area Check Valid Failed since one of the componentToCheckIfWithin is a Sphere Component, but the PaintWithinArea Shape is not set to be sphereShape!")), FColor::Red);
					...
					if (compToCheckWithinTemp.paintWithinAreaShape != Enum_PaintWithinAreaShape::isSquareOrRectangleShape) {
						...
						UVertexPaintFunctionLibrary::PrintTaskLog(paintWithinAreaStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint Within Area Check Valid Failed since one of the componentToCheckIfWithin is a Box Component, but the PaintWithinArea Shape is not set to be Square or Rectangle Shape!")), FColor::Red);
					...
					if (compToCheckWithinTemp.paintWithinAreaShape != Enum_PaintWithinAreaShape::isComplexShape) {
						...
						UVertexPaintFunctionLibrary::PrintTaskLog(paintWithinAreaStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint Within Area Check Valid Failed since one of the componentToCheckIfWithin is a Skeletal Mesh Component, but the PaintWithinArea Shape is not set to be Complex!")), FColor::Red);
					...
					if (compToCheckWithinTemp.paintWithinAreaShape != Enum_PaintWithinAreaShape::isComplexShape) {
						...
						UVertexPaintFunctionLibrary::PrintTaskLog(paintWithinAreaStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint Within Area Check Valid Failed since one of the componentToCheckIfWithin is a Spline Mesh Component, but the PaintWithinArea Shape is not set to be Complex!")), FColor::Red);
				...
				UVertexPaintFunctionLibrary::PrintTaskLog(paintWithinAreaStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint Within Area Check Valid Fail! One of the componentsToCheckIfIsWithin is Not Valid! ")), FColor::Red);
	...
	if (!passedChecks_Local) {
		...
		UVertexPaintFunctionLibrary::PrintTaskLog(paintWithinAreaStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Check Valid Failed at Paint on Mesh Within Area!")), FColor::Red);
	...
	if (paintWithinAreaStruct.applyVertexColorSettings.applyColorsUsingPhysicsSurface.applyVertexColorUsingPhysicsSurface) {
		...
		UVertexPaintFunctionLibrary::PrintTaskLog(paintWithinAreaStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint on Mesh Within Area CheckValid Successfull. Trying to Apply Color with EPhysicalSurface: %s  with Paint Strength: %f  -  Fall Back to RGBA Colors if Failed to Apply Colors Using Physics Surface: %i  -  Paint Strength on Surfaces Without the Physical Surface: %f"), *StaticEnum<EPhysicalSurface>()->GetDisplayNameTextByValue(paintWithinAreaStruct.applyVertexColorSettings.applyColorsUsingPhysicsSurface.physicsSurfaceToApply.GetValue()).ToString(), paintWithinAreaStruct.applyVertexColorSettings.applyColorsUsingPhysicsSurface.paintStrengthToApply, paintWithinAreaStruct.applyVertexColorSettings.applyColorsUsingPhysicsSurface.fallbackToRGBAColorsIfFailedToApplyColorsUsingPhysicsSurface, paintWithinAreaStruct.applyVertexColorSettings.applyColorsUsingPhysicsSurface.paintStrengtOnChannelsWithoutThePhysicsSurface), FColor::Green);
	}

	else {

		UVertexPaintFunctionLibrary::PrintTaskLog(paintWithinAreaStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint on Mesh Within Area CheckValid Successfull. Trying to Apply Color: %s"), *FLinearColor(paintWithinAreaStruct.applyVertexColorSettings.redColor, paintWithinAreaStruct.applyVertexColorSettings.greenColor, paintWithinAreaStruct.applyVertexColorSettings.blueColor, paintWithinAreaStruct.applyVertexColorSettings.alphaColor).ToString()), FColor::Green);
	}


	for (auto compsToCheckIfWithinTemp : paintWithinAreaStruct.componentsToCheckIfIsWithin) {
		...
		if (auto boxComp = Cast<UBoxComponent>(compsToCheckIfWithinTemp.componentToCheckIfIsWithin)) {
			...
			if (paintWithinAreaStruct.debugSettings.drawPaintWithinAreaDebugSymbols)
				DrawDebugBox(paintWithinAreaStruct.meshComponent->GetWorld(), compsToCheckIfWithinTemp.componentToCheckIfIsWithin_Bounds.GetBox().GetCenter(), compsToCheckIfWithinTemp.componentToCheckIfIsWithin_Bounds.GetBox().GetExtent(), FColor::Red, false, paintWithinAreaStruct.debugSettings.drawPaintWithinAreaDebugSymbolsDuration, 0, 5);
		}
				...
				if (compsToCheckIfWithinTemp.paintWithinAreaShape == Enum_PaintWithinAreaShape::isSquareOrRectangleShape) {
					...
					if (compsToCheckIfWithinTemp.componentToCheckIfIsWithin->GetBodySetup()) {
						...
						if (compsToCheckIfWithinTemp.componentToCheckIfIsWithin->GetBodySetup()->AggGeom.BoxElems.Num() > 1) {
							...
							UVertexPaintFunctionLibrary::PrintTaskLog(paintWithinAreaStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint Within Area with Square or Rectangle for Static Mesh Component. NOTE That the Static Mesh has more than 1 Box Collision under it. If they're not centered then then it's not a Square or Rectangle anymore and the Result may not be the expected one. ")), FColor::Orange);
						}
					}
		...
		compsToCheckIfWithinTemp.componentToCheckIfIsWithin_LocationInMeshComponentToPaintSpace = UKismetMathLibrary::InverseTransformLocation(paintWithinAreaStruct.meshComponent->GetComponentTransform(), compsToCheckIfWithinTemp.componentToCheckIfIsWithin_WorldTransform.GetLocation());

		compsToCheckIfWithinTemp.componentToCheckIfIsWithin_RotationInMeshComponentToPaintSpace = UKismetMathLibrary::InverseTransformRotation(paintWithinAreaStruct.meshComponent->GetComponentTransform(), compsToCheckIfWithinTemp.componentToCheckIfIsWithin_WorldTransform.GetRotation().Rotator());

		compsToCheckIfWithinTemp.componentToCheckIfIsWithin_ScaleInMeshComponentToPaintSpace = UKismetMathLibrary::InverseTransformLocation(paintWithinAreaStruct.meshComponent->GetComponentTransform(), compsToCheckIfWithinTemp.componentToCheckIfIsWithin_WorldTransform.GetScale3D());
...
void UVertexPaintDetectionComponent::PaintOnEntireMesh(FVertexPaintOnEntireMeshStruct paintOnEntireMeshStruct, FVertexDetectAdditionalDataToPassThrough additionalDataToPassThrough) {
	...
	if (IsValid(paintOnEntireMeshStruct.meshComponent)) {
		...
		if (IsValid(paintOnEntireMeshStruct.meshComponent->GetOwner())) {
			...
			UVertexPaintFunctionLibrary::PrintTaskLog(paintOnEntireMeshStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Paint Entire Mesh on Actor: %s and Component: %s"), *paintOnEntireMeshStruct.meshComponent->GetOwner()->GetName(), *paintOnEntireMeshStruct.meshComponent->GetName()), FColor::Cyan);
		}
	}

	// Clamps it so if the user sets like 0.000001, we will paint at the lowest amount of 1 when converted to FColor which range from 0-255
	paintOnEntireMeshStruct.applyVertexColorSettings = UVertexPaintFunctionLibrary::ClampPaintStrengths(paintOnEntireMeshStruct.applyVertexColorSettings);
	...
	paintOnEntireMeshStruct.applyVertexColorSettings = UVertexPaintFunctionLibrary::ClampPaintLimits(paintOnEntireMeshStruct.applyVertexColorSettings);
	...
	if (!UVertexPaintFunctionLibrary::CheckValidFundementals(paintOnEntireMeshStruct, paintOnEntireMeshStruct.meshComponent))
		passedChecks_Local = false;

	if (!UVertexPaintFunctionLibrary::CheckValidVertexPaintSettings(paintOnEntireMeshStruct))
		passedChecks_Local = false;

	if (!UVertexPaintFunctionLibrary::CheckValidVertexPaintColorSettings(paintOnEntireMeshStruct))
		passedChecks_Local = false;
	...
	if (passedChecks_Local && IsValid(paintOnEntireMeshStruct.meshComponent)) {
		...
		if (paintOnEntireMeshStruct.paintOnRandomVerticesSettings.paintAtRandomVerticesSpreadOutOverTheEntireMesh) {
			...
			if (paintOnEntireMeshStruct.paintOnRandomVerticesSettings.paintAtRandomVerticesSpreadOutOverTheEntireMesh_PercentToPaint <= 0) {
				...
				UVertexPaintFunctionLibrary::PrintTaskLog(paintOnEntireMeshStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Check Valid, Paint at Entire Mesh Fail: Set to Paint Random Vertices Spread Out Over the Entire Mesh but the Percent to Spread out is 0 when it should be between 0-100. ")), FColor::Red);
			...
			if (paintOnEntireMeshStruct.paintOnRandomVerticesSettings.paintAtRandomVerticesSpreadOutOverTheEntireMesh) {
				...
				if (numberOfVertices_Local > 0) {
					...
					if (UKismetMathLibrary::MapRangeClamped(paintOnEntireMeshStruct.paintOnRandomVerticesSettings.paintAtRandomVerticesSpreadOutOverTheEntireMesh_PercentToPaint, 0, 100, 0, numberOfVertices_Local) < 1) {
						...
						UVertexPaintFunctionLibrary::PrintTaskLog(paintOnEntireMeshStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Check Valid, Paint at Entire Mesh Fail: Set to Paint Random Vertices Spread Out Over the Entire Mesh but the Percent set to Spread out is so low that not even a single vertex at LOD0 would get painted.")), FColor::Red);
				...
				UVertexPaintFunctionLibrary::PrintTaskLog(paintOnEntireMeshStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint Entire Mesh Set to Randomize Vertices to Paint Within Area of Effect: %f"), paintOnEntireMeshStruct.paintOnRandomVerticesSettings.onlyRandomizeWithinAreaOfEffectAtLocation_AreaOfEffect), FColor::Cyan);
			}
		}
	}
	...
	if (!passedChecks_Local) {
		...
		UVertexPaintFunctionLibrary::PrintTaskLog(paintOnEntireMeshStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Check Valid Failed at Paint on Entire Mesh!")), FColor::Red);
	...
	if (paintOnEntireMeshStruct.applyVertexColorSettings.applyColorsUsingPhysicsSurface.applyVertexColorUsingPhysicsSurface) {
		...
		UVertexPaintFunctionLibrary::PrintTaskLog(paintOnEntireMeshStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint on Entire Mesh CheckValid Successfull. Trying to Apply Color with EPhysicalSurface: %s  with Paint Strength: %f  -  Fall Back to RGBA Colors if Failed to Apply Colors Using Physics Surface: %i  -  Paint Strength on Surfaces Without the Physical Surface: %f"), *StaticEnum<EPhysicalSurface>()->GetDisplayNameTextByValue(paintOnEntireMeshStruct.applyVertexColorSettings.applyColorsUsingPhysicsSurface.physicsSurfaceToApply.GetValue()).ToString(), paintOnEntireMeshStruct.applyVertexColorSettings.applyColorsUsingPhysicsSurface.paintStrengthToApply, paintOnEntireMeshStruct.applyVertexColorSettings.applyColorsUsingPhysicsSurface.fallbackToRGBAColorsIfFailedToApplyColorsUsingPhysicsSurface, paintOnEntireMeshStruct.applyVertexColorSettings.applyColorsUsingPhysicsSurface.paintStrengtOnChannelsWithoutThePhysicsSurface), FColor::Green);
	}

	else {

		UVertexPaintFunctionLibrary::PrintTaskLog(paintOnEntireMeshStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint on Entire Mesh CheckValid Successfull. Trying to Apply Color: %s"), *FLinearColor(paintOnEntireMeshStruct.applyVertexColorSettings.redColor, paintOnEntireMeshStruct.applyVertexColorSettings.greenColor, paintOnEntireMeshStruct.applyVertexColorSettings.blueColor, paintOnEntireMeshStruct.applyVertexColorSettings.alphaColor).ToString()), FColor::Green);
	}
...
void UVertexPaintDetectionComponent::PaintColorSnippetOnMesh(FVertexPaintColorSnippetStruct paintColorSnippetStruct, FVertexDetectAdditionalDataToPassThrough additionalDataToPassThrough) {
	...
	if (IsValid(paintColorSnippetStruct.meshComponent)) {
		...
		if (IsValid(paintColorSnippetStruct.meshComponent->GetOwner())) {
			...
			UVertexPaintFunctionLibrary::PrintTaskLog(paintColorSnippetStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Paint Color Snippet on Actor: %s and Component: %s"), *paintColorSnippetStruct.meshComponent->GetOwner()->GetName(), *paintColorSnippetStruct.meshComponent->GetName()), FColor::Cyan);
		}
	}
	...
	if (!UVertexPaintFunctionLibrary::CheckValidFundementals(paintColorSnippetStruct, paintColorSnippetStruct.meshComponent))
		passedChecks_Local = false;

	if (!UVertexPaintFunctionLibrary::CheckValidVertexPaintSettings(paintColorSnippetStruct))
		passedChecks_Local = false;
	...
	if (passedChecks_Local && !paintColorSnippetStruct.colorSnippetTag.IsValid() && !paintColorSnippetStruct.randomizeAnyColorSnippetTagUnderThisCategory.IsValid()) {
		...
		UVertexPaintFunctionLibrary::PrintTaskLog(paintColorSnippetStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - PaintColorSnippet Failed because no Snippet Tag was provided and it's not set to Randomize a Snippet Tag either!")), FColor::Red);
	...
	if (!passedChecks_Local) {
		...
		UVertexPaintFunctionLibrary::PrintTaskLog(paintColorSnippetStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Check Valid Failed at Paint Color Snippet!")), FColor::Red);
	...
	UVertexPaintFunctionLibrary::PrintTaskLog(paintColorSnippetStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint Color Snippet CheckValid Successfull. ")), FColor::Green);
	...
	if (paintColorSnippetStruct.colorSnippetID == paintColorSnippetStruct.colorSnippetTag.GetTagName().ToString()) {
		...
		if (availableColorSnippetTags_Local.FindRef(paintColorSnippetStruct.colorSnippetTag).ToSoftObjectPath().ToString().Len() > 0) {
			...
			UVertexPaintFunctionLibrary::PrintTaskLog(paintColorSnippetStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint Color Snippet Fail as we the Snippet Found didn't point to a valid color snippet data asset soft pointer. ")), FColor::Red);
		}
	}

	else {

		UVertexPaintFunctionLibrary::PrintTaskLog(paintColorSnippetStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint Color Snippet Fail as the Color Snippet ID didn't get set correctly. The Tag you tried to Paint is probably not Registered to this Mesh. If you know it is, try saving and restarting the editor. ")), FColor::Red);
	}


	if (!successfullyStartedAsyncLoadRequest) {

		UVertexPaintFunctionLibrary::PrintTaskLog(paintColorSnippetStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Failed to Start Color Snippet Async Load Request and thus the Task!")), FColor::Red);
...
void UVertexPaintDetectionComponent::PaintColorSnippetOnMesh_FinishedLoadingColorSnippetDataAsset(int finishedID, FVertexPaintColorSnippetStruct paintColorSnippetStruct, FVertexDetectAdditionalDataToPassThrough additionalDataToPassThrough) {
	...
	if (passedChecks_Local && !paintColorSnippetStruct.colorSnippetTag.IsValid() && !paintColorSnippetStruct.randomizeAnyColorSnippetTagUnderThisCategory.IsValid()) {
		...
		UVertexPaintFunctionLibrary::PrintTaskLog(paintColorSnippetStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - PaintColorSnippet Failed after Finished Loading because no Snippet Tag was provided and it's not set to Randomize a Snippet Tag either!")), FColor::Red);
	...
	if (!passedChecks_Local) {
		...
		UVertexPaintFunctionLibrary::PrintTaskLog(paintColorSnippetStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Check Valid Failed at Paint Color Snippet after Finishing Loading Color Snippet Data Asset!")), FColor::Red);
	...
	if (finishedStreamableHandle_Local.Get()) {
			...
			if (auto colorSnippetDataAsset_Local = Cast<UVertexPaintColorSnippetDataAsset>(finishedStreamableHandle_Local.Get()->GetLoadedAsset())) {
				...
				if (colorSnippetDataAsset_Local->snippetColorData.Contains(paintColorSnippetStruct.colorSnippetID)) {
					...
					if (colorSnippetDataAsset_Local->snippetColorData.FindRef(paintColorSnippetStruct.colorSnippetID).colorSnippetDataPerLOD.IsValidIndex(0)) {
						...
						UVertexPaintFunctionLibrary::PrintTaskLog(paintColorSnippetStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - PaintColorSnippet Fail! - The Color Snippet JSon Per LOD doesn't have a valid LOD0 index. ")), FColor::Red);
					}
				}
				...
				UVertexPaintFunctionLibrary::PrintTaskLog(paintColorSnippetStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - PaintColorSnippet Fail! - The Color Snippet Data Asset could not be loaded. If you have deleted it after storing snippets on it then recommend opening up the Editor Utility widget which will refresh and clear up if any mesh has references to old data assets ")), FColor::Red);
			}
		}
		...
		UVertexPaintFunctionLibrary::PrintTaskLog(paintColorSnippetStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - PaintColorSnippet Fail! - After Attempting to Load the Color Snippet Data Asset the FStreamableHandle wasn't Valid! ")), FColor::Red);
	}
	...
	UVertexPaintFunctionLibrary::PrintTaskLog(paintColorSnippetStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint Color Snippet Load Color Snippet Data Asset Successfull. Trying to Apply Snippet: %s"), *paintColorSnippetStruct.colorSnippetID), FColor::Green);
