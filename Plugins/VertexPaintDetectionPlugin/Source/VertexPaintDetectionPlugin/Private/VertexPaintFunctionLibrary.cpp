...
void UVertexPaintFunctionLibrary::PrintTaskLog(FVertexDetectFundementalStruct fundementalStruct, const FString text, FColor screenTextColor) {

	if (fundementalStruct.taskWorld && (fundementalStruct.debugSettings.printLogsToScreen || fundementalStruct.debugSettings.printLogsToOutputLog)) {

		UKismetSystemLibrary::PrintString(fundementalStruct.taskWorld, text, fundementalStruct.debugSettings.printLogsToScreen, fundementalStruct.debugSettings.printLogsToOutputLog, screenTextColor, fundementalStruct.debugSettings.printLogsToScreen_Duration);
	}
}
...
void UVertexPaintFunctionLibrary::SetMeshComponentVertexColors_Wrapper(UPrimitiveComponent* meshComponent, FVertexPaintSetMeshComponentVertexColors setMeshComponentVertexColorsSettings, FVertexDetectAdditionalDataToPassThrough additionalDataToPassThrough) {
	...
	if (IsValid(meshComponent)) {
		...
		UVertexPaintFunctionLibrary::PrintTaskLog(setMeshComponentVertexColorsSettings, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Set Mesh Component Vertex Colors on Actor: %s and Component: %s"), *setMeshComponentVertexColorsSettings.meshComponent->GetOwner()->GetName(), *setMeshComponentVertexColorsSettings.meshComponent->GetName()), FColor::Cyan);
	}
	...
	if (!UVertexPaintFunctionLibrary::CheckValidFundementals(setMeshComponentVertexColorsSettings, setMeshComponentVertexColorsSettings.meshComponent))
		passedChecks_Local = false;

	if (passedChecks_Local && !UVertexPaintFunctionLibrary::CheckValidVertexPaintSettings(setMeshComponentVertexColorsSettings))
		passedChecks_Local = false;
	...
	if (setMeshComponentVertexColorsSettings.vertexColorsAtLOD0ToSet.Num() <= 0) {
		...
		UVertexPaintFunctionLibrary::PrintTaskLog(setMeshComponentVertexColorsSettings, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Set Mesh Component Vertex Colors but the color array passed in are 0 in length!")), FColor::Red);
	...
	if (!passedChecks_Local) {
		...
		UVertexPaintFunctionLibrary::PrintTaskLog(setMeshComponentVertexColorsSettings, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Check Valid Failed for Set Mesh Component Vertex Colors")), FColor::Red);
...
void UVertexPaintFunctionLibrary::SetMeshComponentVertexColorsUsingSerializedString_Wrapper(UPrimitiveComponent* meshComponent, FVertexPaintSetMeshComponentVertexColorsUsingSerializedString setMeshComponentVertexColorsUsingSerializedStringSettings, FVertexDetectAdditionalDataToPassThrough additionalDataToPassThrough) {
	...
	if (IsValid(meshComponent)) {
		...
		UVertexPaintFunctionLibrary::PrintTaskLog(setMeshComponentVertexColorsUsingSerializedStringSettings, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Set Mesh Component Vertex Colors Using Serialized String on Actor: %s and Component: %s"), *setMeshComponentVertexColorsUsingSerializedStringSettings.meshComponent->GetOwner()->GetName(), *setMeshComponentVertexColorsUsingSerializedStringSettings.meshComponent->GetName()), FColor::Cyan);
	}
	...
	if (!UVertexPaintFunctionLibrary::CheckValidFundementals(setMeshComponentVertexColorsUsingSerializedStringSettings, setMeshComponentVertexColorsUsingSerializedStringSettings.meshComponent))
		passedChecks_Local = false;
	...
	if (setMeshComponentVertexColorsUsingSerializedStringSettings.serializedColorDataAtLOD0.Len() <= 0) {
		...
		UVertexPaintFunctionLibrary::PrintTaskLog(setMeshComponentVertexColorsUsingSerializedStringSettings, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Set Mesh Component Vertex Colors Using Serialized String but the serializedColorData String passed in are 0 in length!")), FColor::Red);
	...
	if (!passedChecks_Local) {
		...
		UVertexPaintFunctionLibrary::PrintTaskLog(setMeshComponentVertexColorsUsingSerializedStringSettings, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Check Valid Failed for Set Mesh Component Vertex Colors Using Serialized String")), FColor::Red);
...
FVertexDetectRGBAStruct UVertexPaintFunctionLibrary::ClampPaintStrengths(FVertexDetectRGBAStruct applyVertexColorSettings) {
...
FVertexDetectRGBAStruct UVertexPaintFunctionLibrary::ClampPaintLimits(FVertexDetectRGBAStruct applyColorSetting) {
...
UPhysicalMaterial* UVertexPaintFunctionLibrary::GetPhysicalMaterialUsingPhysicsSurface_Wrapper(const UObject* WorldContextObject, TSubclassOf<UPhysicalMaterial> physicalMaterialClass, TEnumAsByte<EPhysicalSurface> physicsSurface) {
	...
	if (UGameplayStatics::GetGameInstance(WorldContextObject)->GetSubsystem<UVertexPaintDetectionGISubSystem>()) {
		...
		if (auto physicsMaterialTemp = UGameplayStatics::GetGameInstance(WorldContextObject)->GetSubsystem<UVertexPaintDetectionGISubSystem>()->GetAllCachedPhysicsMaterialAssetss().FindRef(physicsSurface)) {
...
void UVertexPaintFunctionLibrary::FillCalculateColorsInfoFundementals(const FVertexDetectFundementalStruct& fundementalStruct, FCalculateColorsInfo& calculateColorsInfo) {
	...
	if (fundementalStruct.multiThreadSettings.useMultithreadingForCalculations) {
		...
		if (calculateColorsInfo.fundementalStruct.debugSettings.atGameThread_drawVertexPositionDebugPoint || calculateColorsInfo.fundementalStruct.debugSettings.atGameThread_drawVertexPositionDebugPointOnVertexThatGotPaintApplied || calculateColorsInfo.fundementalStruct.debugSettings.atGameThread_drawClothVertexPositionDebugPoint || calculateColorsInfo.fundementalStruct.debugSettings.atGameThread_drawVertexNormalDebugArrow) {
			...
			UVertexPaintFunctionLibrary::PrintTaskLog(fundementalStruct, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint/Detect Task is set to Draw Debug Symbols at Vertices but it only works if not Multithreading, i.e. on the Game Thread.")), FColor::Orange);
		}
	}
...
bool UVertexPaintFunctionLibrary::CheckValidFundementals(FVertexDetectFundementalStruct taskFundementals, UPrimitiveComponent* component) {
	...
	if (!IsInGameThread()) {
		...
		UVertexPaintFunctionLibrary::PrintTaskLog(taskFundementals, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Start a Task but not from Game Thread!")), FColor::Red);
	...
	if (!IsValid(component->GetOwner()->GetLevel()->GetOuter())) {
		...
		UVertexPaintFunctionLibrary::PrintTaskLog(taskFundementals, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Actor not in valid sub level!")), FColor::Red);
	...
	if (!staticMeshComp && !skelMeshComp) {
		...
		UVertexPaintFunctionLibrary::PrintTaskLog(taskFundementals, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Paint/Detect but Component %s is not a Static, Spline or Skeletal Mesh!"), *component->GetName()), FColor::Red);
	...
	if (staticMeshComp) {
		...
		if (!IsValid(staticMeshComp->GetStaticMesh())) {
			...
			UVertexPaintFunctionLibrary::PrintTaskLog(taskFundementals, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Paint/Detect on Static Mesh Component %s but it has no Mesh is Set"), *staticMeshComp->GetName()), FColor::Red);
		...
		if (!staticMeshComp->GetStaticMesh()->bAllowCPUAccess) {
			...
			UVertexPaintFunctionLibrary::PrintTaskLog(taskFundementals, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Paint/Detect on Static Mesh but it's CPU Access is not set to True")), FColor::Red);
		...
		if (staticMeshComp->GetStaticMesh().Get()->HasValidNaniteData()) {
			...
			UVertexPaintFunctionLibrary::PrintTaskLog(taskFundementals, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Paint/Detect on Static Mesh that has Nanite Enabled! Vertex Painting on Nanite Meshes is currently not supported. ")), FColor::Red);
		...
		if (!staticMeshComp->GetBodySetup()) {
			...
			UVertexPaintFunctionLibrary::PrintTaskLog(taskFundementals, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Paint/Detect on Static Mesh but it doesn't have a Body Setup")), FColor::Red);

			return false;
		}


		UVertexPaintFunctionLibrary::PrintTaskLog(taskFundementals, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Paint/Detect on Static Mesh Component, with Source Mesh: %s"), *staticMeshComp->GetStaticMesh()->GetName()), FColor::Cyan);
	}
		...
		if (!IsValid(skelMesh_Local)) {
			...
			UVertexPaintFunctionLibrary::PrintTaskLog(taskFundementals, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Paint/Detect on Skeletal Mesh Component but source mesh is null")), FColor::Red);
			...
			UVertexPaintFunctionLibrary::PrintTaskLog(taskFundementals, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Paint/Detect on Skeletal Mesh Component, with Source Mesh: %s"), *skelMesh_Local->GetName()), FColor::Cyan);
		}


		if (!skelMeshComp->GetSkeletalMeshRenderData()) {

			UVertexPaintFunctionLibrary::PrintTaskLog(taskFundementals, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Paint/Detect on Skeletal Mesh but it hasn't properly been initialized yet because it's Skeletal Mesh Render Data isn't valid. ")), FColor::Red);
		...
		if (!skelMeshComp->GetSkeletalMeshRenderData()->IsInitialized()) {
			...
			UVertexPaintFunctionLibrary::PrintTaskLog(taskFundementals, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Paint/Detect on Skeletal Mesh %s but it's SkeletalMeshRenderData isn't Initialized yet. "), *component->GetOwner()->GetName()), FColor::Red);
		...
		if (!skelMesh_Local->GetResourceForRendering()) {
			...
			UVertexPaintFunctionLibrary::PrintTaskLog(taskFundementals, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Paint/Detect on Skeletal Mesh %s but it has invalid ResourceForRendering!"), *component->GetOwner()->GetName()), FColor::Red);
		...
		for (int currentLOD_Local = 0; currentLOD_Local < skelMeshComp->GetSkeletalMeshRenderData()->LODRenderData.Num(); currentLOD_Local++) {
			...
			if (skelMeshRenderData_Local.HasClothData()) {
				...
				if (!skelMeshRenderData_Local.ClothVertexBuffer.IsInitialized()) {
					...
					UVertexPaintFunctionLibrary::PrintTaskLog(taskFundementals, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Paint/Detect on Skeletal Mesh with Cloth but cloth vertex buffer hasn't properly been initialized yet, i.e. the paint job is run to early. ")), FColor::Red);
			...
			if (!skelMeshRenderData_Local.StaticVertexBuffers.StaticMeshVertexBuffer.IsInitialized() || !skelMeshRenderData_Local.StaticVertexBuffers.StaticMeshVertexBuffer.TangentsVertexBuffer.IsInitialized() || !skelMeshRenderData_Local.StaticVertexBuffers.StaticMeshVertexBuffer.GetTangentData()) {
				...
				UVertexPaintFunctionLibrary::PrintTaskLog(taskFundementals, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Paint/Detect on Skeletal Mesh but it hasn't properly been initialized yet, i.e. the paint job is run to early. ")), FColor::Red);
...
bool UVertexPaintFunctionLibrary::CheckValidVertexPaintSettings(FVertexPaintStruct vertexPaintSettings) {
	...
	if (vertexPaintSettings.overrideLOD.overrideLODToPaintUpTo) {
		...
		if (vertexPaintSettings.overrideLOD.amountOfLODsToPaint <= 0) {
			...
			UVertexPaintFunctionLibrary::PrintTaskLog(vertexPaintSettings, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Paint and Override LOD to paint on but LOD given is <= 0!")), FColor::Red);
	...
	if (vertexPaintSettings.canOnlyApplyPaintOnTheseActors.Num() > 0) {
		...
		if (!vertexPaintSettings.canOnlyApplyPaintOnTheseActors.Contains(vertexPaintSettings.actor)) {
			...
			UVertexPaintFunctionLibrary::PrintTaskLog(vertexPaintSettings, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Actor: %s  isn't in the canOnlyApplyPaintOnTheseActors array that has been set. Either add the actor to it, or don't fill the array."), *vertexPaintSettings.actor->GetName()), FColor::Red);
			...
			if (skelMeshComp->GetSkeletalMeshRenderData()->LODRenderData.IsValidIndex(skelMeshComp->GetPredictedLODLevel())) {
				...
				if (skelMeshComp->GetSkeletalMeshRenderData()->LODRenderData[skelMeshComp->GetPredictedLODLevel()].HasClothData()) {
					...
					if (!skelMeshComp->bWaitForParallelClothTask && vertexPaintSettings.affectClothPhysics) {
						...
						UVertexPaintFunctionLibrary::PrintTaskLog(vertexPaintSettings, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Detect/Paint on Skeletal Mesh with Cloth but the Skeletal Mesh Component bWaitForParallelClothTask is false!")), FColor::Red);
...
bool UVertexPaintFunctionLibrary::CheckValidVertexPaintColorSettings(FVertexPaintColorSettingStruct colorSettings) {
	...
	if (colorSettings.applyVertexColorSettings.applyColorsUsingPhysicsSurface.applyVertexColorUsingPhysicsSurface) {
		...
		if (!GetVertexPaintMaterialDataAsset()) {
			...
			UVertexPaintFunctionLibrary::PrintTaskLog(colorSettings, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Set to Apply Colors Using Physics Surface, but no Material Data Asset is set in the Project Settings. This means that we can't get what Physics Surface is on each Vertex Color Channel. ")), FColor::Red);
		...
		if (colorSettings.applyVertexColorSettings.applyColorsUsingPhysicsSurface.paintStrengthToApply == 0 && colorSettings.applyVertexColorSettings.applyColorsUsingPhysicsSurface.paintStrengtOnChannelsWithoutThePhysicsSurface == 0 && colorSettings.applyVertexColorSettings.applyVertexColorSetting == EApplyVertexColorSetting::EAddVertexColor) {
			...
			UVertexPaintFunctionLibrary::PrintTaskLog(colorSettings, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Set to Apply Colors Using Physics Surface, but Paint Strength Multiplier is 0. ")), FColor::Red);
		...
		if (colorSettings.applyVertexColorSettings.applyColorsUsingPhysicsSurface.physicsSurfaceToApply == EPhysicalSurface::SurfaceType_Default) {
			...
			UVertexPaintFunctionLibrary::PrintTaskLog(colorSettings, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Set to Apply Colors Using Physics Surface, but it's set to Default. ")), FColor::Red);
		...
		if (!applyUsingPhysicsSurface_GotColorsToApplyOnMaterial) {
			...
			if (colorSettings.applyVertexColorSettings.applyColorsUsingPhysicsSurface.fallbackToRGBAColorsIfFailedToApplyColorsUsingPhysicsSurface) {
				...
				UVertexPaintFunctionLibrary::PrintTaskLog(colorSettings, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Set to Apply Colors Using Physics Surface, but was unable to get Colors To Apply on any of the Mesh's Materials. Double check if the physics surfaces is registered to it, as well as physics surface families is setup correctly. Is set to Fallback to RGBA Colors so will check if those will make a difference.")), FColor::Orange);
			}
			else {

				UVertexPaintFunctionLibrary::PrintTaskLog(colorSettings, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Set to Apply Colors Using Physics Surface, but was unable to get Colors To Apply on any of the Mesh's Materials, and is set to Not fallback to the RGBA Colors. Double check if the physics surfaces is registered to it, as well as physics surface families is setup correctly. ")), FColor::Red);
	...
	if (colorSettings.applyVertexColorSettings.applyVertexColorSetting == EApplyVertexColorSetting::EAddVertexColor && (!colorSettings.applyVertexColorSettings.applyColorsUsingPhysicsSurface.applyVertexColorUsingPhysicsSurface || (colorSettings.applyVertexColorSettings.applyColorsUsingPhysicsSurface.applyVertexColorUsingPhysicsSurface && colorSettings.applyVertexColorSettings.applyColorsUsingPhysicsSurface.fallbackToRGBAColorsIfFailedToApplyColorsUsingPhysicsSurface && !applyUsingPhysicsSurface_GotColorsToApplyOnMaterial))) {
		...
		if (colorSettings.applyVertexColorSettings.redColor == 0 && colorSettings.applyVertexColorSettings.greenColor == 0 && colorSettings.applyVertexColorSettings.blueColor == 0 && colorSettings.applyVertexColorSettings.alphaColor == 0) {
			...
			UVertexPaintFunctionLibrary::PrintTaskLog(colorSettings, FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Trying to Apply Colors with RGBA with 0 Strength in Colors which wouldn't make any difference. ")), FColor::Red);
...
bool UVertexPaintFunctionLibrary::CheckHaveReachedMaxDetectionTask(FVertexDetectFundementalStruct fundementalSettings) {
