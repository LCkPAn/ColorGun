...
void FVertexPaintDetectionAsyncTasks::DoWork() {
	...
	if (IsValid(calculateColorsInfo_Global.getAllVertexColorsSettings.actor)) {
		...
		if (calculateColorsInfo_Global.setMeshComponentVertexColorsUsingSerializedStringSettings.actor) {
			...
			if (rapidJsonStringToDeSerialize.Len() > 0) {
				...
				calculateColorsResultMessage.Add(FAsyncTaskDebugMessages(FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Set Mesh Colors with Serialized String Task Fail as the String to De-Serialize and get colors from is 0 in Length!")), FColor::Red));
			}
		}
	...
	if (IsInGameThread()) {
		...
		TArray<FAsyncTaskDebugMessages> calculateColorsResultMessage_Local = calculateColorsResultMessage;

		// Broadcasts on Game Thread so the actual Applying of colors happens there
		AsyncTask(ENamedThreads::GameThread, [calculateColorsInfoTemp, paintTasResultTemp, calculateColorsResultMessage_Local]() {
			...
			if (UVertexPaintFunctionLibrary::IsWorldValid(calculateColorsInfoTemp.worldTaskWasCreatedIn)) {
				...
				if (IsValid(gameInstance_Local)) {
					...
					if (IsValid(gameInstanceSubsystem_Local)) {
						...
						gameInstanceSubsystem_Local->CalculateColorsTaskFinished(calculateColorsInfoTemp, paintTasResultTemp, calculateColorsResultMessage_Local);
					}
				}
			}
			});
...
EVertexPaintDetectionTaskResult FVertexPaintDetectionAsyncTasks::CalculateColorToApply(bool updateMeshVertexColorData) {
	...
	for (int currentLOD_Local = 0; currentLOD_Local < lodsToLoopThrough_Local; currentLOD_Local++) {
		...
		float distanceFromVertexToHitLocation = 0;
		...
		if (calculateColorsInfo_Global.paintOnEntireMeshSettings.actor && (calculateColorsInfo_Global.paintOnEntireMeshSettings.paintOnRandomVerticesSettings.onlyRandomizeWithinAreaOfEffectAtLocation || (calculateColorsInfo_Global.paintOnEntireMeshSettings.paintOnRandomVerticesSettings.paintAtRandomVerticesSpreadOutOverTheEntireMesh && calculateColorsInfo_Global.paintOnEntireMeshSettings.paintOnRandomVerticesSettings.paintAtRandomVerticesSpreadOutOverTheEntireMesh_PercentToPaint > 0))) {
			...
			if (calculateColorsInfo_Global.paintOnEntireMeshSettings.paintOnRandomVerticesSettings.onlyRandomizeWithinAreaOfEffectAtLocation) {
				...
				calculateColorsResultMessage.Add(FAsyncTaskDebugMessages(FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Paint Entire Mesh Set to Randomize %i Amount of Vertices over the Mesh on LOD: %i   -   Total Amount of Verts: %i"), (int)paintEntireMesh_Random_AmountOfVerticesLeftToRandomize_Local, currentLOD_Local, colorFromLOD_Local.Num()), FColor::Cyan));
			}
		}

		calculateColorsResultMessage.Add(FAsyncTaskDebugMessages(FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - CalculateColorToApply() - Amount of Sections to Loop Through: %i"), sectionsToLoopThrough_Local.Num()), FColor::Cyan));
		...
		for (auto currentSection_Local : sectionsToLoopThrough_Local) {
			...
			if (IsValid(calculateColorsInfo_Global.vertexPaintStaticMeshComponent)) {
				...
				if (!skelMeshRenderData_Local.RenderSections.IsValidIndex(currentSection_Local)) {
					...
					calculateColorsResultMessage.Add(FAsyncTaskDebugMessages(FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Async Task Failed because invalid Render Section!")), FColor::Red));
			...
			// Resets this here, so in case we're painting using phys surface, and comes to a section After one where we got colors to apply and limit, but not on the current, it will not have old stuff from previous section. 
			paintLimitRGBA_Local = FVertexPaintingLimitRGBA();
			...
			if (calculateColorsInfo_Global.paintOnMeshColorSettings.applyVertexColorSettings.applyColorsUsingPhysicsSurface.applyVertexColorUsingPhysicsSurface) {
				...
				if (successfullyGotVertexColorsToApplyTemp) {
					...
					paintLimitRGBA_Local.usePaintLimits = calculateColorsInfo_Global.paintOnMeshColorSettings.applyVertexColorSettings.applyColorsUsingPhysicsSurface.physicsSurfacePaintLimit.usePaintLimits;

					paintLimitRGBA_Local.limitColorIfTheColorWasAlreadyOverTheLimit = calculateColorsInfo_Global.paintOnMeshColorSettings.applyVertexColorSettings.applyColorsUsingPhysicsSurface.physicsSurfacePaintLimit.limitColorIfTheColorWasAlreadyOverTheLimit;
					...
					if (redColorToApplySuccessfullyGotTemp) {
						...
						paintLimitRGBA_Local.paintLimitOnRedChannel = calculateColorsInfo_Global.paintOnMeshColorSettings.applyVertexColorSettings.applyColorsUsingPhysicsSurface.physicsSurfacePaintLimit.paintLimitOnPhysicsSurface;
					...
					if (greenColorToApplySuccessfullyGotTemp) {
						...
						paintLimitRGBA_Local.paintLimitOnGreenChannel = calculateColorsInfo_Global.paintOnMeshColorSettings.applyVertexColorSettings.applyColorsUsingPhysicsSurface.physicsSurfacePaintLimit.paintLimitOnPhysicsSurface;
					...
					if (blueColorToApplySuccessfullyGotTemp) {
						...
						paintLimitRGBA_Local.paintLimitOnBlueChannel = calculateColorsInfo_Global.paintOnMeshColorSettings.applyVertexColorSettings.applyColorsUsingPhysicsSurface.physicsSurfacePaintLimit.paintLimitOnPhysicsSurface;
					...
					if (alphaColorToApplySuccessfullyGotTemp) {
						...
						paintLimitRGBA_Local.paintLimitOnAlphaChannel = calculateColorsInfo_Global.paintOnMeshColorSettings.applyVertexColorSettings.applyColorsUsingPhysicsSurface.physicsSurfacePaintLimit.paintLimitOnPhysicsSurface;
				...
				// If it's not registered or the Material isn't valid, and it's set to only apply colors if it's registered, then we just set the colors to 0, so no difference will occur at least for the section with this particular Material
					...
					if (materialToGetSurfacesFrom) {
						...
						calculateColorsResultMessage.Add(FAsyncTaskDebugMessages(FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Set to Apply Colors using Physics Surface but GetColorsToApplyFromPhysicsSurface Failed on Material: %s. Make sure Physics Surface Parent/Childs are Setup Correctly, as well as correct Physics Surfaces on the Vertex Colors Channels for the Material if you want to apply colors on it with this physics surface. onlyApplyColorsIfPhysicsSurfaceIsRegistered is True so no colors will be changed. "), *materialToGetSurfacesFrom->GetName()), FColor::Red));
					}


					// If on the last section and haven't been able to get colors to apply on any material, then the task fails as no difference will occur
					if (!applyUsingPhysicsSurface_GotColorsToApplyOnMaterial && currentSection_Local == sectionsToLoopThrough_Local.Num() - 1) {

						calculateColorsResultMessage.Add(FAsyncTaskDebugMessages(FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Set to Apply Colors using Physics Surface but was unable to get Colors to Apply on Any of the Mesh's Materials, and is set to Not fall back to RGBA Colors if unable to get the colors, so the Task Fails as no difference will occur. ")), FColor::Red));
			...
			// If not set to paint using physics surface but is a paint job
			else if (calculateColorsInfo_Global.paintOnMeshColorSettings.actor) {

				paintLimitRGBA_Local = calculateColorsInfo_Global.paintOnMeshColorSettings.applyVertexColorSettings.vertexColorChannelsPaintLimit;
			...
			// If Regular Paint Job, i.e. not a color snippet, then checks if we should even apply colors after the applyVertexColorUsingPhysicsSurface, if not, and there's no reason to loop through vertices then we can skip this section since no difference will occur anyway. currentLODVertex_Local gets set earlier to be whatever the min of the section is so it should match the next section we skip to. But if things like use paint limits etc. is set to be true, which we check in PaintJobShouldLoopThroughVerticesAndLODS(), then we will still loop through the verts. 
			if (calculateColorsInfo_Global.paintOnMeshColorSettings.actor) {
			...
			for (uint32 currentSectionVertex_Local = 0; currentSectionVertex_Local < (uint32)sectionMaxAmountOfVerts_Local; currentSectionVertex_Local++) {
				...
				if (calculateColorsInfo_Global.vertexPaintStaticMeshComponent) {
					...
					if (skelMeshRenderData_Local.RenderSections[currentSection_Local].HasClothingData() && currentLOD_Local == calculateColorsInfo_Global.vertexPaintSkelComponent->GetPredictedLODLevel()) {
						...
						if (!skelMeshRenderData_Local.ClothVertexBuffer.IsInitialized()) {
							...
							calculateColorsResultMessage.Add(FAsyncTaskDebugMessages(FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Async Task Failed skeletal mesh with cloth's cloth vertex buffer isn't initialized")), FColor::Red));
				...
				if (calculateColorsInfo_Global.paintOnMeshAtLocationSettings.actor) {
					...
					distanceFromVertexToHitLocation = (hitLocation_WorldSpace_Paint_Local - currentVertexWorldSpace).Size();
				...
				if (calculateColorsInfo_Global.fundementalStruct.debugSettings.atGameThread_drawVertexNormalDebugArrow && !calculateColorsInfo_Global.fundementalStruct.multiThreadSettings.useMultithreadingForCalculations)
					DrawDebugLine(calculateColorsInfo_Global.vertexPaintDetectionHitActor->GetWorld(), currentVertexWorldSpace, currentVertexWorldSpace + (normalsFromLOD_Local[currentLODVertex_Local] * 25), FColor::Red, false, calculateColorsInfo_Global.fundementalStruct.debugSettings.atGameThread_DrawVertexPointsDuration, 0, .5);

				if (calculateColorsInfo_Global.fundementalStruct.debugSettings.atGameThread_drawVertexPositionDebugPoint && !calculateColorsInfo_Global.fundementalStruct.multiThreadSettings.useMultithreadingForCalculations)
					DrawDebugPoint(calculateColorsInfo_Global.vertexPaintDetectionHitActor->GetWorld(), currentVertexWorldSpace, 5, FColor::Red, false, calculateColorsInfo_Global.fundementalStruct.debugSettings.atGameThread_DrawVertexPointsDuration, 0);
				...
				if (currentLOD_Local == 0) {
					...
					if (calculateColorsInfo_Global.paintOnMeshAtLocationSettings.actor || (calculateColorsInfo_Global.detectInfoSettings.actor && !calculateColorsInfo_Global.getAllVertexColorsSettings.actor)) {
						...
						if (calculateColorsInfo_Global.paintOnMeshAtLocationSettings.actor) {
							...
							if (distanceFromVertexToHitLocation < closestDistanceToClosestVertex_Paint_Local) {


								// If its less then updates distanceFromTraceHitToVertPos, so in the next loop it will check against that value
								closestDistanceToClosestVertex_Paint_Local = distanceFromVertexToHitLocation;
					...
					if (shouldApplyVertexColors_Local) {
						...
						if (WillAppliedColorMakeChangeOnVertex(colorFromLOD_Local[currentLODVertex_Local], currentVertexWorldSpace, normalsFromLOD_Local[currentLODVertex_Local], calculateColorsInfo_Global.paintOnMeshColorSettings)) {
							...
							if (calculateColorsInfo_Global.paintOnMeshAtLocationSettings.actor) {
								...
								if (distanceFromVertexToHitLocation >= calculateColorsInfo_Global.paintOnMeshAtLocationSettings.paintAtAreaSettings.areaOfEffectMin && distanceFromVertexToHitLocation <= calculateColorsInfo_Global.paintOnMeshAtLocationSettings.paintAtAreaSettings.areaOfEffectMax) {
									...
									if (cureentVertexNormalToHitNormalDot_Paint >= vertexNormalMinDotToHitNormalToGetColored_MinDot_Local) {
										...
										paintAtLocation_FallOffRange_Global = 0;
										paintAtLocation_DistianceFromFalloffBase_Global = 0;
										paintAtLocation_ScaleFalloffFrom_Global = 0;
										

										if (calculateColorsInfo_Global.paintOnMeshAtLocationSettings.paintAtAreaSettings.fallOffType == EVertexPaintAtLocationFallOffType::InwardFallOff) {

											paintAtLocation_DistianceFromFalloffBase_Global = distanceFromVertexToHitLocation;
											paintAtLocation_FallOffRange_Global = calculateColorsInfo_Global.paintOnMeshAtLocationSettings.paintAtAreaSettings.areaOfEffectMin;
											paintAtLocation_ScaleFalloffFrom_Global = calculateColorsInfo_Global.paintOnMeshAtLocationSettings.paintAtAreaSettings.distanceToBaseFalloffFrom;
										}

										else if (calculateColorsInfo_Global.paintOnMeshAtLocationSettings.paintAtAreaSettings.fallOffType == EVertexPaintAtLocationFallOffType::OutwardFallOff) {

											paintAtLocation_DistianceFromFalloffBase_Global = distanceFromVertexToHitLocation;
											paintAtLocation_FallOffRange_Global = calculateColorsInfo_Global.paintOnMeshAtLocationSettings.paintAtAreaSettings.areaOfEffectMax;
											paintAtLocation_ScaleFalloffFrom_Global = calculateColorsInfo_Global.paintOnMeshAtLocationSettings.paintAtAreaSettings.distanceToBaseFalloffFrom;
										}
										
										else if (calculateColorsInfo_Global.paintOnMeshAtLocationSettings.paintAtAreaSettings.fallOffType == EVertexPaintAtLocationFallOffType::SphericalFallOff) {

											float distanceTOBaseFalloffFromTemp = calculateColorsInfo_Global.paintOnMeshAtLocationSettings.paintAtAreaSettings.distanceToBaseFalloffFrom;
											paintAtLocation_FallOffRange_Global = calculateColorsInfo_Global.paintOnMeshAtLocationSettings.paintAtAreaSettings.areaOfEffectMin - calculateColorsInfo_Global.paintOnMeshAtLocationSettings.paintAtAreaSettings.areaOfEffectMax;

											if (distanceFromVertexToHitLocation > distanceTOBaseFalloffFromTemp)
												paintAtLocation_DistianceFromFalloffBase_Global = distanceTOBaseFalloffFromTemp - distanceFromVertexToHitLocation;

											else if (distanceFromVertexToHitLocation < distanceTOBaseFalloffFromTemp)
												paintAtLocation_DistianceFromFalloffBase_Global = distanceFromVertexToHitLocation - distanceTOBaseFalloffFromTemp;
										}

										colorFromLOD_Local[currentLODVertex_Local] = GetColorToApplyOnVertex(colorFromLOD_Local[currentLODVertex_Local], currentVertexWorldSpace, normalsFromLOD_Local[currentLODVertex_Local], currentBoneName_Local, calculateColorsInfo_Global.paintOnMeshAtLocationSettings.paintAtAreaSettings.fallOffStrength, paintAtLocation_FallOffRange_Global, calculateColorsInfo_Global.paintOnMeshAtLocationSettings.paintAtAreaSettings.distanceToBaseFalloffFrom, paintAtLocation_ScaleFalloffFrom_Global, calculateColorsInfo_Global.paintOnMeshAtLocationSettings.paintAtAreaSettings.areaOfEffectMin, calculateColorsInfo_Global.paintOnMeshAtLocationSettings.paintAtAreaSettings.areaOfEffectMax, paintAtLocation_DistianceFromFalloffBase_Global, calculateColorsInfo_Global.paintOnMeshColorSettings, vertexColorGotChanged_Local, applyColorsOnRedChannel_Local, applyColorsOnGreenChannel_Local, applyColorsOnBlueChannel_Local, applyColorsOnAlphaChannel_Local, paintLimitRGBA_Local);
								...
								for (int i = 0; i < calculateColorsInfo_Global.paintOnMeshWithinArea_ComponentsToCheckIfIsWithinInfo.Num(); i++) {
									...
									if (IsVertexWithinArea(currentVertexWorldSpace, calculateColorsInfo_Global.paintOnMeshWithinAreaSettings, calculateColorsInfo_Global.paintOnMeshWithinArea_ComponentsToCheckIfIsWithinInfo[i])) {
										...
										paintWithinArea_DistanceFromVertex_Local -= calculateColorsInfo_Global.paintOnMeshWithinAreaSettings.distanceToBaseFalloffFrom;

										if (paintWithinArea_DistanceFromVertex_Local < 0)
											paintWithinArea_DistanceFromVertex_Local = 0;


										colorFromLOD_Local[currentLODVertex_Local] = GetColorToApplyOnVertex(colorFromLOD_Local[currentLODVertex_Local], currentVertexWorldSpace, normalsFromLOD_Local[currentLODVertex_Local], currentBoneName_Local, calculateColorsInfo_Global.paintOnMeshWithinAreaSettings.fallOffStrength, paintWithinArea_AreaOfEffects_Local[i], 0, 0, 0, paintWithinArea_AreaOfEffects_Local[i], paintWithinArea_DistanceFromVertex_Local, calculateColorsInfo_Global.paintOnMeshColorSettings, vertexColorGotChanged_Local, applyColorsOnRedChannel_Local, applyColorsOnGreenChannel_Local, applyColorsOnBlueChannel_Local, applyColorsOnAlphaChannel_Local, paintLimitRGBA_Local);
								...
								if (paintEntireMesh_PaintCurrentVertex_Local) {
									...
									colorFromLOD_Local[currentLODVertex_Local] = GetColorToApplyOnVertex(colorFromLOD_Local[currentLODVertex_Local], currentVertexWorldSpace, normalsFromLOD_Local[currentLODVertex_Local], currentBoneName_Local, 0, 0, 0, 0, 0, 0, 0, calculateColorsInfo_Global.paintOnMeshColorSettings, vertexColorGotChanged_Local, applyColorsOnRedChannel_Local, applyColorsOnGreenChannel_Local, applyColorsOnBlueChannel_Local, applyColorsOnAlphaChannel_Local, paintLimitRGBA_Local);
							...
							if (triedToApplyColorOnVertex_Local) {
								...
								if (calculateColorsInfo_Global.fundementalStruct.debugSettings.atGameThread_drawVertexPositionDebugPointOnVertexThatGotPaintApplied && !calculateColorsInfo_Global.fundementalStruct.multiThreadSettings.useMultithreadingForCalculations)
									DrawDebugPoint(calculateColorsInfo_Global.vertexPaintDetectionHitActor->GetWorld(), currentVertexWorldSpace, 7, FColor::Green, false, calculateColorsInfo_Global.fundementalStruct.debugSettings.atGameThread_DrawVertexPointsDuration, 0);
							}
						}
	...
	calculateColorsResultMessage.Add(FAsyncTaskDebugMessages(FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - CalculateColorsToApply() - Finished Looping through all LODs")), FColor::Cyan));
	...
	if (updateMeshVertexColorData && vertexDetectMeshData_Global.meshDataPerLOD.Num() > 1 && calculateColorsInfo_Global.propogateLOD0ToAllLODs) {
		...
		calculateColorsResultMessage.Add(FAsyncTaskDebugMessages(FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Propogating LOD0 Colors to LODs using Modified Engine Method on Mesh Component: %s"), *calculateColorsInfo_Global.vertexPaintComponent->GetName()), FColor::Cyan));
...
bool FVertexPaintDetectionAsyncTasks::PaintJobShouldLoopThroughVerticesAndLODS(FVertexPaintColorSettingStruct paintColorSettings) {
	...
	return (paintColorSettings.applyVertexColorSettings.vertexColorChannelsPaintLimit.usePaintLimits || (paintColorSettings.applyVertexColorSettings.applyColorsUsingPhysicsSurface.applyVertexColorUsingPhysicsSurface && paintColorSettings.applyVertexColorSettings.vertexColorChannelsPaintLimit.usePaintLimits) || paintColorSettings.onlyAffectVerticesWithDirectionToLocation.onlyAffectVerticesWithDirectionToActorOrLocation || ShouldLoopThroughVerticesAndLODs(paintColorSettings.callbackSettings, paintColorSettings.meshComponent));
}
...
EVertexPaintDetectionTaskResult FVertexPaintDetectionAsyncTasks::IsTaskValid(const int& currentLOD) {
	...
	if (IsValid(calculateColorsInfo_Global.vertexPaintStaticMeshComponent)) {
		...
		if (!skelMesh_Local->GetResourceForRendering()->LODRenderData.IsValidIndex(currentLOD)) {
			...
			calculateColorsResultMessage.Add(FAsyncTaskDebugMessages(FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Async Task Failed, trying to access invalid LOD from GetResourceForRendering->LODRenderData!")), FColor::Red));
		...
		if (!calculateColorsInfo_Global.vertexPaintSkelComponent->GetSkeletalMeshRenderData()->LODRenderData.IsValidIndex(currentLOD)) {
			...
			calculateColorsResultMessage.Add(FAsyncTaskDebugMessages(FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Async Task Failed, trying to access invalid LOD from GetSkeletalMeshRenderData->LODRenderData")), FColor::Red));
		...
		if (!skelMeshRenderData_Local.StaticVertexBuffers.StaticMeshVertexBuffer.IsInitialized() || !skelMeshRenderData_Local.StaticVertexBuffers.StaticMeshVertexBuffer.TangentsVertexBuffer.IsInitialized() || !skelMeshRenderData_Local.StaticVertexBuffers.StaticMeshVertexBuffer.GetTangentData()) {
			...
			calculateColorsResultMessage.Add(FAsyncTaskDebugMessages(FString::Printf(TEXT("Runtime Vertex Color Paint and Detection Plugin - Async Task Failed because static mesh tangent vertex buffer isn't initialized!")), FColor::Red));
...
FColor FVertexPaintDetectionAsyncTasks::GetColorToApplyOnVertex(const FColor & currentVertexColor, const FVector & currentVertexWorldPosition, const FVector & currentVertexNormal, const FName & currentBoneName, const float& fallOffStrength, const float& fallOffRange, const float& distanceToBaseFalloffFrom, const float& scaleFallOffFrom, const float& areaOfEffectMin, const float& areaOfEffectMax, const float& distanceFromFalloffBaseToVertexLocation, const FVertexPaintColorSettingStruct & paintOnMeshColorSetting, bool& vertexGotColorChanged, const bool& applyColorOnRedChannel, const bool& applyColorOnGreenChannel, const bool& applyColorOnBlueChannel, const bool& applyColorOnAlphaChannel, FVertexPaintingLimitRGBA paintLimitOnRGBA) {
	...
	if (paintLimitOnRGBA.usePaintLimits) {
		...
		GetVertexColorAfterAnyLimitation(paintLimitOnRGBA.limitColorIfTheColorWasAlreadyOverTheLimit, (float)currentVertexColor.R, getColorToApplyOnVertex_ColorRed_Global, UKismetMathLibrary::MapRangeClamped(paintLimitOnRGBA.paintLimitOnRedChannel, 0, 1, 0, 255), (float)currentVertexColor.G, getColorToApplyOnVertex_ColorGreen_Global, UKismetMathLibrary::MapRangeClamped(paintLimitOnRGBA.paintLimitOnGreenChannel, 0, 1, 0, 255), (float)currentVertexColor.B, getColorToApplyOnVertex_ColorBlue_Global, UKismetMathLibrary::MapRangeClamped(paintLimitOnRGBA.paintLimitOnBlueChannel, 0, 1, 0, 255), (float)currentVertexColor.A, getColorToApplyOnVertex_ColorAlpha_Global, UKismetMathLibrary::MapRangeClamped(paintLimitOnRGBA.paintLimitOnAlphaChannel, 0, 1, 0, 255), getColorToApplyOnVertex_ColorRed_Global, getColorToApplyOnVertex_ColorGreen_Global, getColorToApplyOnVertex_ColorBlue_Global, getColorToApplyOnVertex_ColorAlpha_Global);
	}
